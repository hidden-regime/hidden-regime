...
class DataValidator:
    ...
    def validate_data(
        ...
        try:
            ...
            if "log_return" in data.columns:
                return_issues, return_warnings, return_metrics = self._validate_returns(
                    data
                )
            ...
            missing_issues, missing_warnings, missing_metrics = (
        ...
        is_valid = (
            len([issue for issue in issues if not issue.startswith("Warning:")]) == 0
        )
    ...
    def _validate_structure(self, data: pd.DataFrame) -> List[str]:
        ...
        if len(data.columns) > 20:
            issues.append(
                f"Too many columns ({len(data.columns)}) - may indicate data issues"
            )
    ...
    def _validate_dates(
        ...
        metrics.update(
            {
                "date_range_days": (
                    (dates.max() - dates.min()).days if len(dates) > 1 else 0
                ),
                "n_dates": len(dates),
        ...
        metrics.update(
            {
                ...
                "max_consecutive_missing": (
                    ...
                    if any(col in data.columns for col in ["price"])
                    else 0
                ),
            }
        )
    ...
    def _validate_volume(self, data: pd.DataFrame) -> Tuple[List[str], Dict[str, Any]]:
        ...
        if zero_volume_pct > 0.1:  # >10% zero volume
            warnings_list.append(
                f"High percentage of zero volume days: {zero_volume_pct:.1%}"
            )
    ...
    def _generate_recommendations(
        ...
        if any("missing" in issue.lower() for issue in issues):
            recommendations.append(
                "Consider data imputation or filtering periods with excessive missing data"
            )

        if any("price" in issue.lower() for issue in issues):
            recommendations.append(
                "Review price data for splits, dividends, or data quality issues"
            )

        # Recommendations based on warnings
        if any("outlier" in warning.lower() for warning in warnings_list):
            recommendations.append(
                "Consider outlier treatment (winsorization, removal, or robust modeling)"
            )

        if any("volatility" in warning.lower() for warning in warnings_list):
            recommendations.append(
                "High volatility detected - consider regime-specific modeling"
            )

        if any("gap" in warning.lower() for warning in warnings_list):
            recommendations.append(
                "Data gaps detected - ensure proper handling of non-trading periods"
            )

        # Recommendations based on data characteristics
        if len(data) < 100:
            recommendations.append(
                "Limited data available - consider longer time period for robust analysis"
            )
    ...
    def _calculate_quality_score(
        self, issues: List[str], warnings_list: List[str], metrics: Dict[str, Any]
    ) -> float:
        ...
        if "outlier_percentage" in metrics:
            score -= (
                metrics["outlier_percentage"] * 1.0
            )  # More strict penalty for outliers
