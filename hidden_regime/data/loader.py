...
class DataLoader:
    ...
    def load_stock_data(
        ...
        use_ohlc_avg = use_ohlc_avg if use_ohlc_avg is not None else self.config.use_ohlc_average
        
        # Check cache first
        cache_key = f"{ticker}_{start_dt.date()}_{end_dt.date()}_{source}_{use_ohlc_avg}"
    ...
    def load_multiple_stocks(
        ...
        **kwargs
        ...
        if failed_tickers:
            warnings.warn(f"Failed to load {len(failed_tickers)} tickers: {failed_tickers}")
    ...
    def _process_raw_data(
        ...
        if use_ohlc_avg and all(col in data.columns for col in ['Open', 'High', 'Low', 'Close']):
            result['price'] = (data['Open'] + data['High'] + data['Low'] + data['Close']) / 4
        else:
    ...
    def plot(self, 
        ...
        try:
            ...
            from ..visualization.plotting import (
                ...
                get_regime_colors, REGIME_COLORS
            )
        except ImportError:
            raise ImportError(
        ...
        for i, (title, plot_subtype) in enumerate(subplot_configs):
            ...
            if plot_subtype == 'price':
                ...
                if len(prices) >= 20:
                    ...
                    ax.plot(dates, ma_20, color='orange', linewidth=1, alpha=0.7, 
                           linestyle='--', label='20-day MA')
                
                if len(prices) >= 50:
                    ma_50 = pd.Series(prices).rolling(50, min_periods=1).mean()
                    ax.plot(dates, ma_50, color=REGIME_COLORS['Crisis'], linewidth=1, alpha=0.7, 
                ...
                if negative_mask.any():
                    ax.scatter(dates[negative_mask], returns[negative_mask], 
                              c=bear_color, marker='v', alpha=0.7, s=20,
                              edgecolors='black', linewidth=0.5, label='Negative Returns')
                
                # Plot positive returns with up triangles
                if positive_mask.any():
                    ax.scatter(dates[positive_mask], returns[positive_mask], 
                              c=bull_color, marker='^', alpha=0.7, s=20,
                              edgecolors='black', linewidth=0.5, label='Positive Returns')
                ...
                if len(returns) >= 20:
                    ...
                    ax_vol = ax.twinx()
                    ax_vol.plot(dates, rolling_vol, color='purple', alpha=0.5, 
                               linewidth=1, label='20-day Vol')
                ...
                ax.hist(returns, bins=50, density=True, alpha=0.7, color='lightblue', 
                ...
                ax.plot(x, normal_dist, 'r-', linewidth=2, alpha=0.8, label='Normal Dist')
                ...
                ax.axvline(mu, color=REGIME_COLORS['Bull'], linestyle='--', alpha=0.8, label=f'Mean: {mu:.4f}')
                ax.axvline(mu + 2*sigma, color=REGIME_COLORS['Crisis'], linestyle=':', alpha=0.6, label=f'+2σ: {mu + 2*sigma:.4f}')
                ax.axvline(mu - 2*sigma, color=REGIME_COLORS['Crisis'], linestyle=':', alpha=0.6, label=f'-2σ: {mu - 2*sigma:.4f}')
                ...
                volume = volume_data
                ax.bar(dates, volume, color=REGIME_COLORS['Sideways'], alpha=0.6, width=1, 
                ...
                if len(volume) >= 20:
                    ...
                    ax.plot(dates, vol_ma, color=REGIME_COLORS['Bull'], linewidth=2, alpha=0.8, 
                           label='20-day Vol MA')
                    ax.legend()
    ...
    def _validate_and_clean_dates(self, dates: pd.Series) -> Tuple[Union[pd.DatetimeIndex, pd.RangeIndex], Optional[np.ndarray]]:
        """
        ...
        try:
            ...
            tz = getattr(dt_index, 'tz', None) or (dt_index.dtype.tz if hasattr(dt_index.dtype, 'tz') else None)
            ...
            if not valid_mask.all():
                ...
                warnings.warn(f"Filtered out {invalid_count} dates outside matplotlib range (0001-9999) or NaT values")
