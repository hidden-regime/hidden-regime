...
class DataPreprocessor:
    ...
    def _detect_outliers(self, returns: pd.Series) -> pd.Series:
        ...
        if self.validation_config.outlier_method == 'zscore':
            ...
            raise ValueError(f"Unknown outlier method: {self.validation_config.outlier_method}")
    ...
    def _winsorize_outliers(
        ...
        if 'log_return' in result.columns:
            ...
            if len(outlier_mask) == len(returns):
                ...
                result['log_return'] = np.clip(returns, lower_percentile, upper_percentile)
    ...
    def _calculate_returns(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.preprocessing_config.return_method == 'log':
            ...
            raise ValueError(f"Unknown return method: {self.preprocessing_config.return_method}")
    ...
    def _apply_smoothing(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'price' in result.columns:
            result['price_smoothed'] = result['price'].rolling(window=window, center=True).mean()
        
        # Smooth returns
        if 'log_return' in result.columns:
            result['log_return_smoothed'] = result['log_return'].rolling(
    ...
    def _align_timestamps(
        ...
        for ticker, df in data_dict.items():
            if 'date' in df.columns:
                ...
                aligned[ticker] = df_reindexed.reset_index().rename(columns={'index': 'date'})
            else:
                aligned[ticker] = df
