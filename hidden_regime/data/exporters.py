...
class StructuredDataExporter:
    ...
    def export_temporal_snapshots(self, temporal_snapshots: List[Dict[str, Any]],
                                 format: str = 'json',
        ...
        if format.lower() == 'json':
            ...
            return self._export_pickle(temporal_snapshots, filename, "temporal_snapshots")  # Use original data
        else:
            raise ValueError(f"Unsupported export format: {format}")

    def export_regime_evolution_results(self, evolution_results: Dict[str, Any],
                                      format: str = 'json',
                                      filename: Optional[str] = None) -> str:
        """
    ...
    def export_signal_attribution_results(self, attribution_results: Dict[str, Any],
                                        format: str = 'json',
                                        filename: Optional[str] = None) -> str:
        """
        ...
        if format.lower() == 'json':
            ...
            return self._export_pickle(attribution_results, filename, "signal_attribution")
        else:
            raise ValueError(f"Unsupported export format: {format}")

    def export_comprehensive_analysis(self, temporal_snapshots: List[Dict[str, Any]],
        ...
        if temporal_snapshots:
            comprehensive_data['temporal_snapshots'] = self._prepare_temporal_data_for_export(temporal_snapshots)
            comprehensive_data['export_metadata']['data_includes'].append('temporal_snapshots')

        # Add evolution results
        if evolution_results:
            comprehensive_data['regime_evolution'] = self._prepare_evolution_data_for_export(evolution_results)
            comprehensive_data['export_metadata']['data_includes'].append('regime_evolution')

        # Add attribution results
        if attribution_results:
            comprehensive_data['signal_attribution'] = self._prepare_attribution_data_for_export(attribution_results)
            comprehensive_data['export_metadata']['data_includes'].append('signal_attribution')

        # Export based on format
        if format.lower() == 'json':
            return self._export_json(comprehensive_data, filename, "comprehensive_analysis")
        elif format.lower() == 'parquet':
            return self._export_parquet(comprehensive_data, filename, "comprehensive_analysis")
        elif format.lower() == 'hdf5':
            return self._export_hdf5(comprehensive_data, filename, "comprehensive_analysis")
        elif format.lower() == 'pickle':
            ...
            return self._export_pickle(original_data, filename, "comprehensive_analysis")
        else:
            raise ValueError(f"Unsupported export format: {format}")

    def export_data_for_reproduction(self, temporal_snapshots: List[Dict[str, Any]],
        ...
        reproduction_data = {
            'reproduction_metadata': {
                ...
                'data_quality_checks': self._perform_data_quality_checks(temporal_snapshots)
            },
    ...
    def _prepare_temporal_data_for_export(self, temporal_snapshots: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for snapshot_data in temporal_snapshots:
            ...
            if snapshot and hasattr(snapshot, 'technical_indicators') and snapshot.technical_indicators:
                exportable_snapshot['technical_indicators'] = snapshot.technical_indicators

            # Convert regime analysis
            if snapshot and hasattr(snapshot, 'regime_analysis') and snapshot.regime_analysis:
        ...
        return {
            ...
            'date_range': {
                'start': exportable_snapshots[0]['timestamp'] if exportable_snapshots else None,
                'end': exportable_snapshots[-1]['timestamp'] if exportable_snapshots else None
            }
        }

    def _prepare_evolution_data_for_export(self, evolution_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'regime_characteristics': evolution_results.get('regime_characteristics', {}),
            'drift_analysis': evolution_results.get('drift_analysis', {}),
            'summary_insights': evolution_results.get('summary_insights', {})
        }

    def _prepare_attribution_data_for_export(self, attribution_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_based_attribution': attribution_results.get('time_based_attribution', {}),
    ...
    def _export_parquet(self, data: Dict[str, Any], filename: str, data_type: str) -> str:
        ...
        pq.write_table(table, output_path, compression='snappy' if self.compress else None)

        self._log_export(str(output_path), 'parquet', data_type, output_path.stat().st_size)
        return str(output_path)
    ...
    def _export_hdf5(self, data: Dict[str, Any], filename: str, data_type: str) -> str:
        ...
        self._log_export(str(output_path), 'hdf5', data_type, output_path.stat().st_size)
        return str(output_path)
    ...
    def _export_pickle(self, data: Any, filename: str, data_type: str) -> str:
        ...
        self._log_export(str(output_path), 'pickle', data_type, output_path.stat().st_size)
        return str(output_path)
    ...
    def _flatten_dict(self, d: Dict[str, Any], parent_key: str, result: Dict[str, Any]) -> None:
    ...
    def _write_to_hdf5_group(self, group: h5py.Group, data: Dict[str, Any], prefix: str) -> None:
    ...
    def _perform_data_quality_checks(self, temporal_snapshots: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        signal_completeness = checks['snapshots_with_signal_data'] / len(temporal_snapshots)
    ...
    def _log_export(self, filepath: str, format: str, data_type: str, file_size: int) -> None:
    ...
    def get_export_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'latest_export': self.export_log[-1]['timestamp'] if self.export_log else None
        }
    ...
    def create_export_manifest(self) -> str:
        ...
        manifest_data = {
            ...
            'supported_formats': {
                'json': {'available': True, 'description': 'JSON format with optional compression'},
                'parquet': {'available': PARQUET_AVAILABLE, 'description': 'Apache Parquet columnar format'},
                'hdf5': {'available': HDF5_AVAILABLE, 'description': 'HDF5 hierarchical data format'},
                'pickle': {'available': True, 'description': 'Python pickle format (preserves exact objects)'}
            }
        }
...
class DataImporter:
    ...
    def _reconstruct_temporal_data_from_df(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
