...
class SufficientStatistics:
    ...
    gamma_sum: np.ndarray = field(default_factory=lambda: np.array([]))  # Sum of state probabilities
    gamma_sum_t1: np.ndarray = field(default_factory=lambda: np.array([]))  # Sum excluding first timestep
    
    # Transition statistics  
    xi_sum: np.ndarray = field(default_factory=lambda: np.array([]))  # Sum of transition probabilities
    
    # Emission statistics
    obs_sum: np.ndarray = field(default_factory=lambda: np.array([]))     # Sum of observations * gamma
    obs_sq_sum: np.ndarray = field(default_factory=lambda: np.array([]))  # Sum of observations^2 * gamma
...
class OnlineHMM(HiddenMarkovModel):
    ...
    def __init__(self, 
        ...
        # Change detection
        self._recent_likelihoods = deque(maxlen=self.online_config.change_detection_window)
        self._baseline_likelihood: Optional[float] = None
        
    def fit(self, returns: Union[np.ndarray, pd.Series], verbose: bool = False) -> 'OnlineHMM':
        """
        ...
        if verbose:
            ...
            print(f"Ready for real-time updates with forgetting factor: {self.online_config.forgetting_factor}")
        
        return self
    
    def _initialize_online_components(self, initial_returns: Union[np.ndarray, pd.Series]):
        ...
        change_window_size = min(len(returns_array), self.online_config.change_detection_window)
        ...
        if self.online_config.enable_change_detection:
            self._baseline_likelihood = self.score(returns_array[-self.online_config.change_detection_window:])
    ...
    def _compute_initial_sufficient_statistics(self, returns: np.ndarray):
        ...
        for t in range(1, T):
            for j in range(self.n_states):
                log_alpha[t, j] = logsumexp(
                    log_alpha[t-1] + np.log(self.transition_matrix_[:, j])
                ) + log_emissions[t, j]
        ...
        self.sufficient_stats.gamma_sum_t1 = np.sum(gamma[1:], axis=0)  # Exclude first timestep
    ...
    def add_observation(self, new_return: float, update_parameters: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            should_update = (
                update_parameters and
                self._observations_since_last_update >= self.online_config.min_observations_for_update
            )
            
            if should_update or change_detected:
                self._update_parameters_online(new_return, current_state_probs, change_detected)
                self._observations_since_last_update = 0
            ...
            regime_info = self._generate_regime_info(current_state_probs, change_detected)
    ...
    def _detect_change_point(self, new_return: float) -> bool:
        ...
        self._baseline_likelihood = (0.95 * self._baseline_likelihood + 
                                   0.05 * current_likelihood)
    ...
    def _update_parameters_online(self, observation: float, state_probs: np.ndarray, change_detected: bool):
        ...
        new_initial_probs, new_transition_matrix, new_emission_params = self._estimate_parameters_from_stats()
        
        # Apply parameter smoothing to prevent instability
        if self.online_config.parameter_smoothing:
            self._apply_parameter_smoothing(new_initial_probs, new_transition_matrix, new_emission_params)
        else:
            # Direct update
    ...
    def _update_sufficient_statistics(self, observation: float, state_probs: np.ndarray):
    ...
    def _estimate_parameters_from_stats(self) -> Tuple[np.ndarray, np.ndarray, List[Tuple[float, float]]]:
        ...
        for i in range(self.n_states):
            ...
            if row_sum > 0:
                new_transition_matrix[i, :] = self.sufficient_stats.xi_sum[i, :] / row_sum
            else:
        ...
        for k in range(self.n_states):
            if self.sufficient_stats.gamma_sum[k] > 0:
                # Mean
                mean = self.sufficient_stats.obs_sum[k] / self.sufficient_stats.gamma_sum[k]
                
                # Variance (with numerical stability)
                mean_sq = self.sufficient_stats.obs_sq_sum[k] / self.sufficient_stats.gamma_sum[k]
    ...
    def _apply_parameter_smoothing(self, new_initial_probs: np.ndarray, 
        ...
        self.initial_probs_ = (weight_old * self.initial_probs_ + 
                              weight_new * new_initial_probs)
        
        # Smooth transition matrix
        self.transition_matrix_ = (weight_old * self.transition_matrix_ + 
                                  weight_new * new_transition_matrix)
    ...
    def _generate_regime_info(self, state_probs: np.ndarray, change_detected: bool) -> Dict[str, Any]:
        ...
        interpretation = get_regime_interpretation(current_regime, self.emission_params_)
    ...
    def __repr__(self) -> str:
        ...
        return (f"OnlineHMM(n_states={self.n_states}, {status}, "
