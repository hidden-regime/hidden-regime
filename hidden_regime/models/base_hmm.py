...
class HiddenMarkovModel:
    ...
    def __init__(
        self, n_states: Optional[int] = None, config: Optional[HMMConfig] = None
    ):
        """
        ...
        if n_states is None:
            ...
            if config is None:
                ...
                if config.n_states == n_states:
                    ...
                    raise ValueError(
                        f"Number of states do not match - config is {config.n_states} and n_states is {n_states}"
                    )
    ...
    def fit(
        ...
        if self.config.auto_select_states:
            try:
                ...
                best_regime_type, validation_score, selection_details = (
                    self._state_standardizer.select_optimal_configuration(
                        returns_array,
                ...
                if verbose:
                    print(
                        f"Auto-selected {best_regime_type} configuration (validation score: {validation_score:.3f})"
                    )
        ...
        if verbose:
            print(
                f"Training {self.n_states}-state HMM on {len(returns_array)} observations"
            )
        ...
        for iteration in range(max_iter):
            try:
                ...
                if log_likelihood < self.config.log_likelihood_threshold:
                    raise HMMTrainingError(
                        f"Extreme log-likelihood at iteration {iteration}: {log_likelihood}"
                    )
                ...
                if verbose and iteration % 10 == 0:
                    print(
                        f"Iteration {iteration}: Log-likelihood = {log_likelihood:.4f}, "
                ...
                if (
                    ...
                    and iteration % self.config.check_convergence_every == 0
                    and check_convergence(log_likelihoods, tol)
                ):
        ...
        if self.config.force_state_ordering and self._state_standardizer is not None:
            try:
                ...
                emission_params, transition_matrix, initial_probs = (
                    self._state_standardizer.reorder_states_by_returns(
                        emission_params, transition_matrix, initial_probs
                    )
                )
        ...
        if self._state_standardizer is not None:
            try:
                self._state_mapping = self._state_standardizer.standardize_states(
                    emission_params
                )

                if self.config.validate_regime_economics:
                    states = self.predict(returns_array)
                    self._standardization_confidence = (
                        self._state_standardizer.validate_interpretation(
                    ...
                    if verbose:
                        print(
                            f"Regime validation confidence: {self._standardization_confidence:.3f}"
                        )
                        if (
                            self._standardization_confidence
                            < self.config.state_validation_threshold
                        ):
                            warnings.warn(
                                f"Low regime validation confidence: {self._standardization_confidence:.3f}"
                            )
        ...
        self.training_history_ = {
            ...
            "final_log_likelihood": (
                log_likelihoods[-1] if log_likelihoods else float("-inf")
            ),
        }
    ...
    def get_current_regime_info(self) -> Dict[str, Any]:
        ...
        if self._current_state_probs is None:
            raise HMMInferenceError(
                "No current state available. Call update_with_observation() first."
            )
        ...
        if self._state_standardizer is not None and self._state_mapping is not None:
            regime_details = (
                self._state_standardizer.get_regime_interpretation_enhanced(
            ...
            regime_interpretation = get_regime_interpretation(
                most_likely_state, self.emission_params_
            )
        ...
        if self._standardization_confidence is not None:
            regime_info["validation_confidence"] = float(
                self._standardization_confidence
            )
    ...
    def analyze_regimes(
        ...
        if (
            hasattr(self, "_state_standardizer")
            and self._state_standardizer is not None
        ):
            ...
            for i in range(self.n_states):
                if hasattr(self, "_state_mapping") and self._state_mapping is not None:
                    ...
                    if isinstance(standardized_state, int):
                        ...
                        config = self._state_standardizer.get_config(
                            self.config.regime_type
                        )
                        if config and standardized_state < len(config.state_names):
                            regime_interpretations[str(i)] = config.state_names[
                                standardized_state
                            ]
                        else:
                            regime_interpretations[str(i)] = get_regime_interpretation(
                                i, self.emission_params_
                            )
                    else:
        ...
        analysis = {
            "model_info": {
                ...
                "standardization_applied": hasattr(self, "_state_standardizer")
                and self._state_standardizer is not None,
            },
            "regime_parameters": {
        ...
        if (
            hasattr(self, "_standardization_confidence")
            and self._standardization_confidence is not None
        ):
    ...
    def save(self, filepath: Union[str, Path]) -> None:
        ...
        try:
            model_data = {
                ...
                "current_state_probs": (
                    self._current_state_probs.tolist()
                    if self._current_state_probs is not None
                    else None
                ),
                "last_observation": self._last_observation,
            ...
            if hasattr(self, "_state_mapping") and self._state_mapping is not None:
                ...
                state_mapping_serializable = {
                    int(k): v for k, v in self._state_mapping.items()
                }
            ...
            if (
                ...
                model_data["standardization_confidence"] = (
                    self._standardization_confidence
                )
    ...
    def load(cls, filepath: Union[str, Path]) -> "HiddenMarkovModel":
        ...
        try:
            ...
            if model_data.get("state_mapping") is not None:
                ...
                state_mapping = {
                    int(k): v for k, v in model_data["state_mapping"].items()
                }
                model._state_mapping = state_mapping
            if model_data.get("standardization_confidence") is not None:
                model._standardization_confidence = model_data[
                    "standardization_confidence"
                ]

            # Recreate state standardizer if needed
            if (
                hasattr(model.config, "regime_type")
                and getattr(model.config, "regime_type", "custom") != "custom"
            ):
                from .state_standardizer import StateStandardizer
    ...
    def plot(
        ...
        for i, (title, plot_subtype) in enumerate(subplot_configs):
            ...
            if plot_subtype == "regimes":
                ...
                plot_returns_with_regimes(
                    ...
                    regime_names,
                    ax,
                    title="Returns Colored by Detected Regime",
                )
                ...
                ax.set_yticklabels(
                    [regime_names.get(i, f"State {i}") for i in range(self.n_states)]
                )
                ...
                regime_stats = self._calculate_regime_stats(
                    returns_array, predicted_states
                )
                ...
                if "log_likelihood_history" in self.training_history_:
                    ...
                    if self.training_history_["converged"]:
                        ...
                        ax.axvline(
                            ...
                            linestyle="--",
                            alpha=0.7,
                            label=f"Converged at iter {final_iter}",
                        )
                        ax.legend()
                else:
                    ax.text(
        ...
        if plot_type == "all":
            summary_text = self._create_model_summary_text(
                returns_array, predicted_states
            )
            fig.text(
                0.02,
    ...
    def _get_regime_names(self) -> Dict[int, str]:
        ...
        if (
            ...
            if config_obj and hasattr(self, "_state_mapping") and self._state_mapping:
                for i in range(self.n_states):
                    mapped_value = self._state_mapping.get(
                        i, self._state_mapping.get(np.int64(i), i)
                    )
                    if isinstance(mapped_value, str):
                        ...
                        regime_names[i] = get_regime_interpretation(
                            i, self.emission_params_
                        )
    ...
    def _calculate_regime_stats(
        self, returns: np.ndarray, states: np.ndarray
    ...
    def _create_model_summary_text(
        self, returns: np.ndarray, states: np.ndarray
    ) -> str:
        ...
        if (
            hasattr(self, "_standardization_confidence")
            and self._standardization_confidence
        ):
            summary.append(
                f"Standardization confidence: {self._standardization_confidence:.3f}"
            )
