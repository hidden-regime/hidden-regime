...
class StreamingDataSource(ABC):
    ...
    ) -> AsyncIterator[StreamingObservation]:
...
class SimulatedDataSource(StreamingDataSource):
    ...
    async def stream_observations(
        ...
        filtered_data = self.historical_data[
            (
                self.historical_data["symbol"].isin(symbols)
                if "symbol" in self.historical_data.columns
                else self.historical_data
            )  # Assume single symbol if no symbol column
        ].sort_values("date")
        ...
        for _, row in filtered_data.iterrows():
            ...
            observation = StreamingObservation(
                ...
                symbol=(
                    symbols[0] if len(symbols) == 1 else row.get("symbol", "UNKNOWN")
                ),
                price=price,
...
class YFinanceLiveDataSource(StreamingDataSource):
    ...
    def __init__(self, poll_interval_seconds: float = 60.0, max_poll_failures: int = 5):
        """
    ...
    async def stream_observations(
        ...
        while self._connected and consecutive_failures < self.max_poll_failures:
            try:
                ...
                logging.warning(
                    f"Polling failed for attempt {consecutive_failures}: {e}"
                )
                await asyncio.sleep(self.poll_interval)
...
class StreamingProcessor:
    ...
    async def start(
        self, symbols: List[str], result_callback: Optional[Callable] = None
    ) -> None:
        """
    ...
    async def _process_real_time(
        self, symbols: List[str], result_callback: Optional[Callable]
    ) -> None:
        ...
        async for observation in self.data_source.stream_observations(symbols):
            ...
            try:
                ...
                if (
                    ...
                    and self._observations_processed
    ...
    async def _process_micro_batch(
        ...
        async for observation in self.data_source.stream_observations(symbols):
            ...
            if len(batch) >= self.config.batch_size:
                ...
                for obs in batch:
                    if (
                        self.config.validate_observations
                        and not self._validate_observation(obs)
                    ):
                        continue
        ...
        for obs in batch:
            if self.config.validate_observations and not self._validate_observation(
                obs
            ):
                continue
            result = await self._process_observation(obs)
    ...
    async def _process_observation(
        self, observation: StreamingObservation
    ) -> StreamingResult:
        ...
        regime_info = self.online_hmm.add_observation(
            observation.log_return, update_parameters=should_update_params
        )
    ...
    def _validate_observation(self, observation: StreamingObservation) -> bool:
        ...
        # Check for outliers
        if (
            abs(observation.log_return) > self.config.outlier_threshold * 0.02
        ):  # 2% baseline std
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "buffer_utilization": len(self._result_buffer)
