...
class PipelineFactory:
    ...
    def create_pipeline(
        ...
        try:
            ...
            observation_component = self.component_factory.create_observation_component(observation_config)
            model_component = self.component_factory.create_model_component(model_config)
            analysis_component = self.component_factory.create_analysis_component(analysis_config)
            
            report_component = None
            if report_config is not None:
                report_component = self.component_factory.create_report_component(report_config)
    ...
    def create_trading_pipeline(
        ...
        return self.create_financial_pipeline(
            ...
            analysis_config_overrides={
                'regime_labels': ['Crisis', 'Bear', 'Sideways', 'Bull'] if n_states == 4 else None,
                'include_trading_signals': True,
                'position_sizing_method': 'volatility_adjusted' if risk_adjustment else 'regime_confidence',
