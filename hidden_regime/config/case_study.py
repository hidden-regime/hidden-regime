...
class CaseStudyConfig:
    ...
    indicators_to_compare: List[str] = None  # Will default to ['rsi', 'macd', 'bollinger_bands', 'moving_average']
    ...
    color_scheme: Literal["classic", "professional", "academic", "pastel", "colorblind_safe", "high_contrast", "blue_red", "viridis", "okabe_ito"] = "colorblind_safe"
    ...
    def __post_init__(self):
        ...
        if self.indicators_to_compare is None:
            self.indicators_to_compare = ['rsi', 'macd', 'bollinger_bands', 'moving_average']

        # Set default output directory if not provided
        if self.output_directory is None:
            self.output_directory = f"./output/case_studies/{self.ticker}_{self.start_date}_{self.end_date}"
    ...
    def validate(self) -> None:
        ...
        if self.n_training <= 0:
            raise ConfigurationError(f"n_training must be positive, got {self.n_training}")

        if self.n_training < 30:
            raise ConfigurationError(f"n_training should be at least 30 days for meaningful analysis, got {self.n_training}")

        # Validate n_states
        if self.n_states < 2 or self.n_states > 10:
            raise ConfigurationError(f"n_states must be between 2 and 10, got {self.n_states}")
        ...
        # Validate indicators
        valid_indicators = ['rsi', 'macd', 'bollinger_bands', 'moving_average', 'stochastic', 'williams_r', 'atr']
        for indicator in self.indicators_to_compare:
            if indicator not in valid_indicators:
                raise ConfigurationError(f"Unknown indicator: {indicator}. Valid indicators: {valid_indicators}")

        # Validate animation parameters
        if self.animation_fps <= 0 or self.animation_fps > 30:
            raise ConfigurationError(f"animation_fps must be between 1 and 30, got {self.animation_fps}")
    ...
    def get_training_date_range(self) -> tuple[str, str]:
        ...
        if self.frequency == "business_days":
            training_start = start_dt - pd.Timedelta(days=int(self.n_training * 1.4))  # Buffer for weekends
            training_start = pd.bdate_range(end=start_dt, periods=self.n_training + 1)[0]
        else:
    ...
    def get_analysis_config_with_overrides(self) -> Dict[str, Any]:
        ...
        base_config = {
            ...
            'indicator_comparisons': self.indicators_to_compare if self.include_technical_indicators else None,
    ...
    def create_quick_study(
        ...
        return cls(
            ...
            end_date=end_date,
            n_training=max(60, days_back // 2),  # Use half the period for training, minimum 60 days
            n_states=n_states,
