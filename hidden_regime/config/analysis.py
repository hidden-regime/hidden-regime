...
class FinancialAnalysisConfig(AnalysisConfig):
    ...
    indicator_comparisons: Optional[List[str]] = field(default_factory=lambda: ["rsi", "macd"])
    ...
    def validate(self) -> None:
        ...
        if self.regime_labels is not None:
            if len(self.regime_labels) != self.n_states:
                raise ConfigurationError(f"regime_labels length {len(self.regime_labels)} must match n_states {self.n_states}")
        ...
        if self.indicator_comparisons is not None:
            valid_indicators = {
                ...
                "price_rate_of_change", "trix", "ultimate_oscillator"
            }
        ...
        valid_methods = ["equal_weight", "regime_confidence", "volatility_adjusted", "risk_parity"]
        if self.position_sizing_method not in valid_methods:
            raise ConfigurationError(f"position_sizing_method must be one of {valid_methods}")
        
        # Validate window parameters
        if self.volatility_window < 5:
            raise ConfigurationError(f"volatility_window must be at least 5, got {self.volatility_window}")
        
        if self.return_window < 5:
            raise ConfigurationError(f"return_window must be at least 5, got {self.return_window}")
        
        if self.minimum_regime_duration < 1:
            raise ConfigurationError(f"minimum_regime_duration must be at least 1, got {self.minimum_regime_duration}")
