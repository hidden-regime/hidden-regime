...
class SimulationConfig(BaseConfig):
    ...
    initial_capital: Optional[float] = None  # If None, calculated from default_shares * first_price
    ...
    # Signal generation
    signal_generators: List[str] = field(default_factory=lambda: ['buy_and_hold', 'hmm_regime_following'])
    hmm_strategy_types: List[str] = field(default_factory=lambda: ['regime_following'])  # regime_following, regime_contrarian, confidence_weighted
    technical_indicators: List[str] = field(default_factory=lambda: ['rsi', 'macd', 'sma'])  # TA library indicators to include
    ...
    calculate_trade_metrics: bool = True  # Whether to calculate detailed trade-level metrics
    ...
    def validate(self) -> None:
        ...
        if self.initial_capital is not None and self.initial_capital <= 0:
            raise ConfigurationError(f"initial_capital must be positive, got {self.initial_capital}")

        if self.default_shares <= 0:
            raise ConfigurationError(f"default_shares must be positive, got {self.default_shares}")

        if self.transaction_cost < 0:
            raise ConfigurationError(f"transaction_cost cannot be negative, got {self.transaction_cost}")

        # Validate risk management parameters
        if not 0 < self.max_position_pct <= 1:
            raise ConfigurationError(f"max_position_pct must be between 0 and 1, got {self.max_position_pct}")

        if not 0 < self.max_portfolio_risk <= 1:
            raise ConfigurationError(f"max_portfolio_risk must be between 0 and 1, got {self.max_portfolio_risk}")

        if not 0 < self.stop_loss_pct <= 1:
            raise ConfigurationError(f"stop_loss_pct must be between 0 and 1, got {self.stop_loss_pct}")

        if not 0 < self.max_total_exposure <= 2:
            raise ConfigurationError(f"max_total_exposure must be between 0 and 2, got {self.max_total_exposure}")
        ...
        valid_hmm_strategies = ['regime_following', 'regime_contrarian', 'confidence_weighted']
        ...
        valid_indicators = [
            'rsi', 'macd', 'sma', 'ema', 'bollinger_bands', 'stochastic', 'williams_r',
            'atr', 'adx', 'aroon', 'cci', 'mfi', 'roc', 'vwap', 'keltner_channels'
        ]
        ...
        if self.risk_free_rate < 0 or self.risk_free_rate > 1:
            raise ConfigurationError(f"risk_free_rate must be between 0 and 1, got {self.risk_free_rate}")

        # Validate trailing stop parameters
        if self.enable_trailing_stops:
            if not 0 < self.trailing_stop_pct <= 1:
                raise ConfigurationError(f"trailing_stop_pct must be between 0 and 1, got {self.trailing_stop_pct}")

        # Validate volatility stop parameters
        if self.enable_volatility_stops:
            if self.volatility_multiplier <= 0:
                raise ConfigurationError(f"volatility_multiplier must be positive, got {self.volatility_multiplier}")
    ...
    def create_aggressive(cls) -> 'SimulationConfig':
        ...
        return cls(
            ...
            technical_indicators=['rsi', 'macd', 'sma', 'ema', 'bollinger_bands', 'stochastic'],
    ...
    def create_comprehensive(cls) -> 'SimulationConfig':
        ...
        return cls(
            ...
            technical_indicators=[
                ...
                'williams_r', 'atr', 'adx', 'aroon', 'cci', 'roc', 'vwap'
            ],
...
class SimulationOutputConfig(BaseConfig):
    ...
    # File formats
    save_formats: List[Literal['csv', 'json', 'parquet']] = field(default_factory=lambda: ['csv', 'json'])
    ...
    def validate(self) -> None:
        ...
        for fmt in self.save_formats:
            if fmt not in valid_formats:
                raise ConfigurationError(f"Invalid save format: {fmt}. Must be one of {valid_formats}")

        # Validate report format
        valid_report_formats = ['markdown', 'html', 'pdf']
        if self.report_format not in valid_report_formats:
            raise ConfigurationError(f"Invalid report format: {self.report_format}. Must be one of {valid_report_formats}")
