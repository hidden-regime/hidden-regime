...
class IndicatorPerformanceComparator:
    ...
    def compare_regime_vs_indicators(
        ...
        if 'predicted_state' not in analysis_results.columns:
            raise ValidationError("Analysis results must contain 'predicted_state' column")
        ...
        for indicator in indicators:
            if indicator in calculated_indicators:
                comparison_results['indicator_analysis'][indicator] = self._analyze_single_indicator(
                    analysis_results['predicted_state'],
                    calculated_indicators[indicator],
                    indicator,
                    analysis_results.get('confidence', pd.Series([1.0] * len(analysis_results)))
                )
        
        # Cross-indicator analysis
        if len(calculated_indicators) > 1:
            comparison_results['cross_indicator_analysis'] = self._analyze_cross_indicators(
                calculated_indicators, analysis_results['predicted_state']
            )
        
        # Predictive performance analysis
        if len(calculated_indicators) > 0:
            comparison_results['predictive_performance'] = self._analyze_predictive_performance(
                analysis_results, calculated_indicators, raw_data
            )
        
        # Regime-indicator agreement matrix
        comparison_results['regime_indicator_matrix'] = self._build_regime_indicator_matrix(
    ...
    def _generate_comparison_summary(self, analysis_results: pd.DataFrame, indicators: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            'date_range': {
                'start': analysis_results.index[0] if hasattr(analysis_results.index, 'to_pydatetime') else 0,
                'end': analysis_results.index[-1] if hasattr(analysis_results.index, 'to_pydatetime') else len(analysis_results)-1
            },
            'regime_summary': {
                'n_states': n_states,
                'state_distribution': analysis_results['predicted_state'].value_counts().to_dict(),
                'avg_confidence': analysis_results.get('confidence', pd.Series([1.0] * len(analysis_results))).mean()
            },
            'indicators_requested': indicators,
            'analysis_timestamp': pd.Timestamp.now()
        }
    
    def _calculate_indicator(self, indicator_name: str, raw_data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            if indicator_name == 'rsi':
                ...
                return self._calculate_williams_r_detailed(raw_data, params.get('period', 14))
            else:
    ...
    def _calculate_rsi_detailed(self, prices: pd.Series, period: int = 14) -> pd.DataFrame:
        ...
        # Generate signals
        signals = np.where(rsi > 70, -1, np.where(rsi < 30, 1, 0))  # -1=sell, 1=buy, 0=hold
        
        # Zone classification
        zones = np.where(rsi > 80, 'extreme_overbought',
                np.where(rsi > 70, 'overbought',
                np.where(rsi > 50, 'bullish',
                np.where(rsi > 30, 'bearish',
    ...
    def _calculate_macd_detailed(self, prices: pd.Series, params: Dict[str, int]) -> pd.DataFrame:
        ...
        momentum = np.where(histogram > histogram.shift(1), 'increasing',
    ...
    def _calculate_bollinger_detailed(self, prices: pd.Series, params: Dict[str, float]) -> pd.DataFrame:
    ...
    def _calculate_ma_detailed(self, prices: pd.Series, period: int = 20) -> pd.DataFrame:
        ...
        # Trend direction
        ma_slope = ma.diff() / ma
        trend = np.where(ma_slope > 0.001, 'uptrend',
    ...
    def _calculate_stochastic_detailed(self, raw_data: pd.DataFrame, params: Dict[str, int]) -> pd.DataFrame:
    ...
    def _calculate_williams_r_detailed(self, raw_data: pd.DataFrame, period: int = 14) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            'signal': signals,
            'zone': np.where(williams_r > -20, 'overbought', 
    ...
    def _analyze_single_indicator(
        ...
        if n_states == 3:
            regime_signals = regime_aligned.map({0: -1, 1: 0, 2: 1})  # Bear, Sideways, Bull
        elif n_states == 4:
            regime_signals = regime_aligned.map({0: -1, 1: -1, 2: 0, 3: 1})  # Crisis, Bear, Sideways, Bull
        else:
            # Default mapping
        ...
        agreement_analysis = {
            'correlation': self._calculate_correlation_metrics(regime_signals, indicator_aligned),
            'classification_metrics': self._calculate_classification_metrics(regime_signals, indicator_aligned),
            ...
            'temporal_analysis': self._analyze_temporal_agreement(regime_signals, indicator_aligned),
            'signal_strength_analysis': self._analyze_signal_strength(
                regime_signals, indicator_data.get('strength', pd.Series([1.0] * len(indicator_data)))
            )
        }
        ...
        return {
            ...
            'summary_statistics': self._generate_indicator_summary_stats(indicator_data)
        }
    
    def _calculate_correlation_metrics(self, regime_signals: pd.Series, indicator_signals: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                'pearson_correlation': float(pearson_corr) if not pd.isna(pearson_corr) else 0.0,
                'spearman_correlation': float(spearman_corr) if not pd.isna(spearman_corr) else 0.0,
                'rolling_correlation_mean': float(rolling_corr.mean()) if not rolling_corr.isna().all() else 0.0,
                'rolling_correlation_std': float(rolling_corr.std()) if not rolling_corr.isna().all() else 0.0,
                'correlation_stability': float(1.0 - rolling_corr.std()) if not rolling_corr.isna().all() else 0.0
            }
        except Exception:
            return {
    ...
    def _calculate_classification_metrics(self, regime_signals: pd.Series, indicator_signals: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            precision = precision_score(regime_classes, indicator_classes, average='weighted', zero_division=0)
            recall = recall_score(regime_classes, indicator_classes, average='weighted', zero_division=0)
            f1 = f1_score(regime_classes, indicator_classes, average='weighted', zero_division=0)
            ...
            if non_neutral_mask.sum() > 0:
                direction_agreement = (regime_classes[non_neutral_mask] == indicator_classes[non_neutral_mask]).mean()
            else:
    ...
    def _calculate_confidence_weighted_agreement(
        ...
        try:
            ...
            agreement_raw = (np.sign(regime_signals) == np.sign(indicator_signals)).astype(float)
            ...
            return {
                ...
                'confidence_correlation': float(confidence.corr(agreement_raw)) if not confidence.corr(agreement_raw) != confidence.corr(agreement_raw) else 0.0
            }
        except Exception:
            return {
    ...
    def _analyze_temporal_agreement(self, regime_signals: pd.Series, indicator_signals: pd.Series) -> Dict[str, Any]:
        """Analyze temporal patterns in agreement."""
        try:
            agreement = (np.sign(regime_signals) == np.sign(indicator_signals)).astype(float)
            ...
            return {
                'rolling_agreement_mean': float(rolling_agreement.mean()) if not rolling_agreement.isna().all() else 0.0,
                'rolling_agreement_std': float(rolling_agreement.std()) if not rolling_agreement.isna().all() else 0.0,
                'agreement_trend': float(agreement_trend),
                'agreement_volatility': float(agreement_volatility) if not pd.isna(agreement_volatility) else 0.0,
    ...
    def _analyze_signal_strength(self, regime_signals: pd.Series, indicator_strength: pd.Series) -> Dict[str, float]:
        """Analyze relationship between signal strength and regime agreement."""
        try:
            agreement = (np.sign(regime_signals) == np.sign(regime_signals)).astype(float)
            ...
            return {
                'strength_agreement_correlation': float(strength_agreement_corr) if not pd.isna(strength_agreement_corr) else 0.0,
                'strong_signal_agreement': float(strong_signal_agreement),
                'avg_signal_strength': float(indicator_strength.mean()),
                'signal_strength_stability': float(1.0 - indicator_strength.std()) if not pd.isna(indicator_strength.std()) else 0.0
            }
        except Exception:
            return {
    ...
    def _rate_indicator_performance(self, agreement_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            weighted_agreement = agreement_analysis['confidence_weighted_agreement']['weighted_agreement']
            
            # Composite score
            composite_score = (abs(correlation) * 0.4 + accuracy * 0.3 + weighted_agreement * 0.3)
    ...
    def _generate_indicator_summary_stats(self, indicator_data: pd.DataFrame) -> Dict[str, Any]:
    ...
    def _analyze_cross_indicators(self, indicators: Dict[str, pd.DataFrame], regime_states: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'consensus_analysis': {
                    'consensus_regime_correlation': regime_states.corr(consensus_signal),
                    'individual_vs_consensus': individual_vs_consensus,
                    'consensus_volatility': float(consensus_signal.std())
                },
                'indicator_diversity': float(correlation_matrix.values[np.triu_indices_from(correlation_matrix.values, k=1)].std())
            }
        except Exception:
            return {}
    ...
    def _analyze_predictive_performance(
        ...
        try:
            ...
            predictive_results['regime_predictive_power'] = self._analyze_regime_predictive_power(
            ...
            for name, data in indicators.items():
                if 'signal' in data.columns:
                    predictive_results['indicator_predictive_power'][name] = self._analyze_indicator_predictive_power(
                        data['signal'], forward_returns, name
                    )
            
            # Compare predictive performance
            predictive_results['comparative_analysis'] = self._compare_predictive_performance(
    ...
    def _analyze_regime_predictive_power(self, regime_states: pd.Series, forward_returns: Dict[str, pd.Series]) -> Dict[str, Any]:
        ...
        for horizon, returns in forward_returns.items():
            ...
            try:
                ...
                for state in regime_aligned.unique():
                    ...
                    if len(state_returns) > 0:
                        regime_returns[f'regime_{state}'] = {
                            ...
                            'sharpe_ratio': float(state_returns.mean() / state_returns.std()) if state_returns.std() > 0 else 0.0,
                ...
                if len(regime_aligned.unique()) > 1:
                    groups = [returns_aligned[regime_aligned == state] for state in regime_aligned.unique()]
    ...
    def _analyze_indicator_predictive_power(self, indicator_signals: pd.Series, forward_returns: Dict[str, pd.Series], indicator_name: str) -> Dict[str, Any]:
            ...
            try:
                ...
                for signal in signals_aligned.unique():
                    ...
                    if len(signal_data) > 0:
                        signal_returns[signal_name] = {
                            ...
                            'sharpe_ratio': float(signal_data.mean() / signal_data.std()) if signal_data.std() > 0 else 0.0,
                ...
                if len(signals_aligned) > 0:
                    ...
                    if total_directional_signals > 0:
                        direction_accuracy = float(correct_directions / total_directional_signals)
                
                results[horizon] = {
                    'signal_returns': signal_returns,
                    'direction_accuracy': direction_accuracy,
                    'signal_correlation': float(signals_aligned.corr(returns_aligned)) if len(signals_aligned) > 1 else 0.0
                }
            except Exception:
                continue
    ...
    def _compare_predictive_performance(self, regime_states: pd.Series, indicators: Dict[str, pd.DataFrame], forward_returns: Dict[str, pd.Series]) -> Dict[str, Any]:
        ...
        try:
            ...
            for horizon, returns in forward_returns.items():
                ...
                if len(regime_aligned) > 1:
                    ...
                    horizon_results['regime_correlation'] = float(regime_correlation) if not pd.isna(regime_correlation) else 0.0
                ...
                for name, data in indicators.items():
                    if 'signal' in data.columns:
                        ...
                        if len(signal_aligned) > 1:
                            ...
                            indicator_correlations[name] = float(correlation) if not pd.isna(correlation) else 0.0
                
                horizon_results['indicator_correlations'] = indicator_correlations
                
                # Best performer
                all_correlations = {'regime': horizon_results.get('regime_correlation', 0.0)}
                all_correlations.update(indicator_correlations)
                
                if all_correlations:
                    best_performer = max(all_correlations.keys(), key=lambda k: all_correlations[k])
    ...
    def _build_regime_indicator_matrix(self, analysis_results: pd.DataFrame, indicators: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        try:
            ...
            for indicator_name, indicator_data in indicators.items():
                if 'signal' in indicator_data.columns:
                    ...
                    contingency = pd.crosstab(regime_aligned, signal_aligned, normalize='index')
    ...
    def _perform_statistical_tests(self, analysis_results: pd.DataFrame, indicators: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    def plot_comparison_results(self, comparison_results: Dict[str, Any], **kwargs) -> plt.Figure:
        ...
        if not comparison_results or 'indicator_analysis' not in comparison_results:
            ...
            ax.text(0.5, 0.5, 'No comparison results available', ha='center', va='center', fontsize=14)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
            ax.axis('off')
            return fig
        ...
        if n_indicators == 0:
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.text(0.5, 0.5, 'No indicators analyzed', ha='center', va='center', fontsize=14)
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 1)
            ax.axis('off')
            return fig
        ...
        scores = [indicator_analysis[ind]['agreement_analysis']['correlation']['pearson_correlation'] 
        ...
        accuracies = [indicator_analysis[ind]['agreement_analysis']['classification_metrics']['accuracy'] 
                     for ind in indicators]
        ...
        if 'cross_indicator_analysis' in comparison_results and 'signal_correlation_matrix' in comparison_results['cross_indicator_analysis']:
            corr_matrix = pd.DataFrame(comparison_results['cross_indicator_analysis']['signal_correlation_matrix'])
            ...
            ax3.text(0.5, 0.5, 'No cross-indicator data', ha='center', va='center', transform=ax3.transAxes)
            ax3.set_title('Indicator Signal Correlations')
        
        # Plot 4: Performance ratings distribution
        ax4 = plt.subplot(3, 2, 4)
        ratings = [indicator_analysis[ind]['performance_rating']['rating'] for ind in indicators]
        ...
        composite_scores = [indicator_analysis[ind]['performance_rating']['composite_score'] 
                           for ind in indicators]
