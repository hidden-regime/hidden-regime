...
try:
    ...
    warnings.warn(
        "matplotlib and/or seaborn not available. Install with: pip install matplotlib seaborn"
    )
...
def format_financial_axis(
    ax, dates: Optional[np.ndarray] = None, date_format: str = "auto"
) -> None:
    """
...
def plot_returns_with_regimes(
    ...
    use_accessibility_features: bool = True,
) -> plt.Axes:
    """
    ...
    markers = (
        get_regime_markers(list(regime_names.values()))
        if use_accessibility_features
        else {}
    )
...
def plot_regime_heatmap(
    ...
    if isinstance(data, pd.DataFrame):
        sns.heatmap(
            data, ax=ax, cmap=cmap, annot=annot, fmt=fmt, cbar_kws={"label": "Value"}
        )
    else:
        ...
        sns.heatmap(
            df, ax=ax, cmap=cmap, annot=annot, fmt=fmt, cbar_kws={"label": "Value"}
        )
    ...
    if plot_type == "bar":
        ...
        if len(metrics) == 1:
            ...
            data_matrix = np.array(
                [
                    [regime_stats[regime].get(metric, 0) for metric in metrics]
...
def save_plot(
    fig: plt.Figure, filepath: str, dpi: int = 150, bbox_inches: str = "tight"
) -> None:
    """
