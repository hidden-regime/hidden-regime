...
class RegimePlotter:
    ...
    def plot_regime_evolution(
        ...
        axes[0].plot(aligned_data.index, aligned_data[price_column],
            ...
            if regime_mask.sum() > 0:
                ...
                for date, confidence in zip(regime_dates, confidence_values):
                    ...
                    axes[0].axvspan(date, date + pd.Timedelta(days=1),
                                  color=regime_colors[regime], alpha=alpha)
        ...
        for regime in range(n_states):
            ...
            rolling_regime_dist[regime] = regime_series.rolling(window=window_size).mean()

        for regime, rolling_dist in rolling_regime_dist.items():
            axes[1].plot(aligned_data.index, rolling_dist,
                        color=regime_colors[regime], linewidth=2,
        ...
        rolling_confidence = aligned_data[confidence_column].rolling(window=window_size).mean()
        axes[2].plot(aligned_data.index, rolling_confidence,
                    color='purple', linewidth=2, label=f'Rolling Confidence ({window_size}d)')
        axes[2].fill_between(aligned_data.index, 0, rolling_confidence,
    ...
    def plot_regime_characteristics_matrix(
        ...
        """
        if metrics is None:
            metrics = ['mean_return', 'volatility', 'max_return', 'min_return', 'skewness']
            ...
            for i, metric in enumerate(metrics):
                for regime in range(n_states):
                    if regime in regime_metrics:
                        metrics_matrix[i, regime] = regime_metrics[regime].get(metric, 0)
...
class PerformancePlotter:
    ...
    def plot_strategy_comparison(
        ...
        if risk_return_data:
            ...
            for i, name in enumerate(names):
                axes[3].annotate(name, (vols[i], rets[i]),
    ...
    def plot_drawdown_analysis(
        ...
        axes[0].plot(cumulative.index, cumulative.values, linewidth=2, color='blue', label='Cumulative Return')
        axes[0].plot(running_max.index, running_max.values, linewidth=1, color='green', alpha=0.7, label='High Water Mark')

        # Shade drawdown periods
        axes[0].fill_between(cumulative.index, cumulative.values, running_max.values,
        ...
        if len(drawdown_values) > 0:
            axes[2].hist(drawdown_values, bins=30, alpha=0.7, color='red', edgecolor='black')
            axes[2].axvline(drawdown.min(), color='darkred', linestyle='--',
                           label=f'Max DD: {drawdown.min():.2%}')
            axes[2].axvline(np.percentile(drawdown_values, 5), color='orange', linestyle='--',
    ...
    def plot_model_comparison(
        ...
        for model_name, model_data in models_data.items():
            if 'regime_data' in model_data:
                ...
                if 'predicted_state' in regime_data.columns:
                    regime_counts = regime_data['predicted_state'].value_counts().sort_index()
                    regime_distributions[model_name] = regime_counts

        if regime_distributions:
            width = 0.8 / n_models
            x_positions = np.arange(max(len(dist) for dist in regime_distributions.values()))

            for i, (model_name, distribution) in enumerate(regime_distributions.items()):
                x_pos = x_positions[:len(distribution)] + i * width
                axes[0].bar(x_pos, distribution.values, width, alpha=0.7,
        ...
        if all('performance' in model_data for model_data in models_data.values()):
            ...
            for i, (model_name, model_data) in enumerate(models_data.items()):
                ...
                axes[1].bar(x_pos + i * width, values, width, alpha=0.7,
                           color=colors[i], label=model_name)
        ...
        for i, (model_name, model_data) in enumerate(models_data.items()):
            if 'performance_series' in model_data:
                ...
                axes[2].plot(cumulative.index, cumulative.values,
                           linewidth=2, color=colors[i], label=model_name)
        ...
        for i, (model_name, model_data) in enumerate(models_data.items()):
            if 'regime_data' in model_data and 'confidence' in model_data['regime_data'].columns:
                confidence = model_data['regime_data']['confidence']
                axes[3].hist(confidence.values, bins=20, alpha=0.5, color=colors[i],
...
class InteractivePlotter:
    ...
    def create_interactive_regime_plot(
        ...
        # Update layout
        fig.update_layout(
            title='Interactive Regime Analysis',
            showlegend=True,
            height=800
        )
...
def create_regime_timeline_plot(
    ...
    for i in range(1, len(regime_data)):
        if regime_data[regime_column].iloc[i] != current_regime or i == len(regime_data) - 1:
            end_date = regime_data.index[i-1] if i < len(regime_data) - 1 else regime_data.index[i]

            # Draw regime period
            duration = (end_date - start_date).days
            axes[0].barh(current_regime, duration, left=start_date, height=0.8,
                        color=regime_colors[current_regime], alpha=0.7,
                        edgecolor='black', linewidth=0.5)

            # Add duration text
            mid_date = start_date + (end_date - start_date) / 2
            axes[0].text(mid_date, current_regime, f'{duration}d',
    ...
    axes[1].plot(regime_data.index, regime_data[confidence_column],
                color='purple', linewidth=2, alpha=0.8)
    axes[1].fill_between(regime_data.index, 0, regime_data[confidence_column],
                        alpha=0.3, color='purple')
    ...
    axes[1].scatter(change_dates, change_confidences, color='red', s=50, alpha=0.8,
                   label='Regime Changes', zorder=5)
...
def create_multi_asset_regime_comparison(
    ...
    for i, (asset_name, (price_data, regime_data)) in enumerate(multi_asset_data.items()):
        ax = axes[i]
        ...
        # Plot price
        ax.plot(aligned_data.index, aligned_data['close'],
