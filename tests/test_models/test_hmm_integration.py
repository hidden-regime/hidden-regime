...
class TestHMMIntegration:
    ...
    def sample_market_data(self):
        ...
        for i in range(1, 100):
            ...
            return_val = np.random.normal(
                regime_means[current_regime], 
                regime_stds[current_regime]
            )
        ...
        data['High'] = data['price'] * (1 + np.abs(np.random.normal(0, 0.005, len(data))))
        data['Low'] = data['price'] * (1 - np.abs(np.random.normal(0, 0.005, len(data))))
    ...
    def test_convenience_function_detect_regimes(self, sample_market_data):
        ...
        returns = np.log(sample_market_data['price'] / sample_market_data['price'].shift(1)).dropna()
    ...
    def test_end_to_end_pipeline_with_hmm(self, mock_ticker, sample_market_data):
        ...
        mock_stock.history.return_value = sample_market_data.set_index('date')[['Open', 'High', 'Low', 'Close', 'Volume']]
        ...
        for i in range(min(5, len(processed_data))):
            regime_info = hmm.update_with_observation(processed_data['log_return'].iloc[i])
    ...
    def test_hmm_with_validation_edge_cases(self):
        ...
        validation_config = ValidationConfig(outlier_method='iqr', outlier_threshold=2.0)
        ...
        hmm = HiddenMarkovModel(n_states=3, config=HMMConfig(random_seed=42, max_iterations=30))
        hmm.fit(processed_data['log_return'], verbose=False)
