...
from hidden_regime.models.streaming import (
    StreamingConfig, StreamingMode, StreamingObservation, StreamingResult,
    StreamingDataSource, SimulatedDataSource, StreamingProcessor
)
...
class TestStreamingConfig:
    ...
    def test_config_modes(self):
        ...
        micro_batch_config = StreamingConfig(mode=StreamingMode.MICRO_BATCH, batch_size=10)
        time_window_config = StreamingConfig(mode=StreamingMode.TIME_WINDOW, time_window_seconds=30.0)
...
class TestStreamingProcessor:
    ...
    async def test_real_time_processing(self, fitted_online_hmm, sample_streaming_data):
        ...
        data_source = SimulatedDataSource(sample_streaming_data, speed_multiplier=1000.0)
        config = StreamingConfig(
    ...
    @pytest.mark.asyncio
    async def test_micro_batch_processing(self, fitted_online_hmm, sample_streaming_data):
    ...
    @pytest.mark.asyncio
    async def test_observation_validation(self, fitted_online_hmm, sample_streaming_data):
    ...
    async def test_rate_limiting(self, fitted_online_hmm, sample_streaming_data):
        ...
        if len(results) >= 3:
            expected_min_time = 2.0 / config.max_observations_per_second  # Time for 3 observations
...
class TestStreamingIntegration:
    ...
    async def test_end_to_end_streaming(self):
        ...
        data_source = SimulatedDataSource(
            historical_data.iloc[50:],  # Stream second half
            speed_multiplier=100.0
        )
