...
class TestHiddenMarkovModel:
    ...
    def test_fit_parameter_override(self, sample_returns):
        ...
        hmm.fit(sample_returns, max_iterations=25, tolerance=1e-3, verbose=False)
    ...
    def test_fit_invalid_data(self):
        ...
        bad_data = np.random.normal(
            0.001, 0.02, 35
        )  # 35 points to ensure >= 30 after removal
        bad_data[5] = np.nan
        bad_data[10] = np.nan
        bad_data[20] = np.nan
    ...
    def test_save_load_pickle(self, sample_returns, config):
        ...
        try:
            ...
            assert np.allclose(
                loaded_hmm._current_state_probs, hmm._current_state_probs
            )
    ...
    def test_training_convergence(self):
        ...
        config = HMMConfig(
            n_states=2, max_iterations=100, tolerance=1e-8, random_seed=42
        )
