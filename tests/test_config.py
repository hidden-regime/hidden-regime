...
from hidden_regime.config.settings import DataConfig, ValidationConfig, PreprocessingConfig
...
class TestPreprocessingConfig:
    ...
    def test_window_size_edge_cases(self):
        ...
        min_config = PreprocessingConfig(
            volatility_window=1,
            smoothing_window=1
        )
...
class TestConfigurationPersistence:
    ...
    def test_config_to_file_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            preprocessing_config_path = os.path.join(temp_dir, 'preprocessing_config.json')
            ...
            assert restored_data_config.max_missing_data_pct == data_config.max_missing_data_pct
            
            assert restored_validation_config.outlier_method == validation_config.outlier_method
            assert restored_validation_config.outlier_threshold == validation_config.outlier_threshold
            
            assert restored_preprocessing_config.return_method == preprocessing_config.return_method
            assert restored_preprocessing_config.volatility_window == preprocessing_config.volatility_window
    ...
    def test_config_inheritance_patterns(self):
        ...
        assert conservative_config.outlier_method == base_config.outlier_method  # Inherited
        assert conservative_config.outlier_threshold == 2.0  # Modified
        assert conservative_config.max_daily_return == 0.2   # Modified
        
        assert aggressive_config.outlier_method == base_config.outlier_method  # Inherited
...
class TestConfigurationDocumentation:
    ...
    def test_dataclass_field_introspection(self):
        ...
        expected_data_fields = {
            'default_source', 'use_ohlc_average', 'include_volume',
            'max_missing_data_pct', 'min_observations', 'cache_enabled',
            'cache_expiry_hours', 'requests_per_minute', 'retry_attempts',
            'retry_delay_seconds'
        }
        ...
        expected_validation_fields = {
            'outlier_method', 'outlier_threshold', 'iqr_multiplier',
            'min_price', 'max_daily_return', 'min_trading_days_per_month',
            'max_consecutive_missing', 'interpolation_method'
        }
        ...
        expected_preprocessing_fields = {
            'return_method', 'apply_smoothing', 'smoothing_window',
            'calculate_volatility', 'volatility_window', 'align_timestamps',
            'fill_method'
        }
