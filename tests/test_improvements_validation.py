...
class TestImprovementsValidation:
    ...
    def test_automatic_financial_pipeline(self, mock_financial_data):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            expected_price = 0.25 * (
                mock_financial_data['Open'] + mock_financial_data['Close'] +
                mock_financial_data['High'] + mock_financial_data['Low']
            )
            ...
            np.testing.assert_array_almost_equal(
                data_output['pct_change'].values,
                expected_pct_change.values,
                decimal=6
            )
            ...
            np.testing.assert_array_almost_equal(
                data_output['log_return'].values,
                expected_log_return.values,
                decimal=6
            )
    ...
    def test_data_driven_state_interpretation(self, mock_financial_data):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            if hasattr(model_component, 'emission_means_') and model_component.emission_means_ is not None:
