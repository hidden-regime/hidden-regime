...
class TestDataPreprocessor:
    ...
    def test_handle_missing_values_excessive(self):
        ...
        with pytest.raises(DataQualityError, match="Too many consecutive missing values"):
            self.preprocessor._handle_missing_values(data_with_excessive_missing)
    
    def test_detect_outliers_iqr(self):
        """Test outlier detection using IQR method."""
        returns = pd.Series([0.01, 0.02, -0.01, 0.15, 0.01, -0.02, 0.01])  # 0.15 is outlier
    ...
    def test_calculate_returns_simple(self):
        ...
        # Test calculation
        expected_simple_return_1 = (result['price'].iloc[1] / result['price'].iloc[0]) - 1
        assert np.isclose(result['simple_return'].iloc[1], expected_simple_return_1, rtol=1e-10)
    
    def test_calculate_volatility_features(self):
        """Test volatility feature calculation."""
        result = self.preprocessor._calculate_volatility_features(self.sample_data.copy())
        ...
        assert pd.isna(result['volatility'].iloc[window-2])  # Just before window is full
    ...
    def test_align_timestamps(self):
        ...
        data2 = create_sample_stock_data(
            n_days=15, 
            start_date=datetime(2023, 1, 8),  # Start later
            price_start=200
        )
