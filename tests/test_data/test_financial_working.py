...
class TestFinancialDataLoaderWorking:
    ...
    def test_load_from_yfinance_retry_logic(self, mock_ticker_class):
        ...
        result = loader._load_from_yfinance("TEST", pd.Timestamp('2023-01-01'), pd.Timestamp('2023-01-02'))
        assert isinstance(result, pd.DataFrame)
        ...
        with pytest.raises(DataLoadError):
            loader._load_from_yfinance("FAIL", pd.Timestamp('2023-01-01'), pd.Timestamp('2023-01-02'))
...
class TestFinancialDataLoaderCoverage:
    ...
    def test_error_message_coverage(self):
        ...
        test_cases = [
            ...
            (pd.DataFrame({'price': [100, -50, 102, 103, 104, 105, 106, 107, 108, 109, 110]}), "Invalid price values"),
        ]
