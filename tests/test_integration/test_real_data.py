        ...
        if len(results_dict) >= 2:
            ...
            for i in range(len(reports)):
                for j in range(i+1, len(reports)):
                    ...
                    assert reports[i] != reports[j], f"Assets {list(results_dict.keys())[i]} and {list(results_dict.keys())[j]} have identical reports"
    ...
    def test_temporal_analysis_real_data(self):
        ...
        try:
            ...
            if len(observation_counts) > 1:
                ...
                assert observation_counts[-1] >= observation_counts[0], "Observation count should increase over time"
        ...
        for signal in signals_to_test:
            try:
                ...
                if signal == 'log_return':
                    ...
                    assert np.mean(regime_entropy) > 0.1, f"Signal {signal} produced too deterministic regimes"
        ...
        for preset_name, config in presets.items():
            try:
                ...
                if preset_name == 'conservative':
                    ...
                    assert regime_change_rate < 0.2, f"Conservative preset had too many regime changes: {regime_change_rate}"
                ...
                # Aggressive should adapt faster (more regime switching allowed)
                    ...
                    assert regime_change_rate < 0.5, f"Aggressive preset had excessive regime changes: {regime_change_rate}"
        ...
        if len(results_by_preset) >= 2:
            ...
            for i in range(len(preset_names)):
                for j in range(i+1, len(preset_names)):
                    ...
                    correlation = np.corrcoef(probs_i.flatten(), probs_j.flatten())[0,1]
                    assert correlation < 0.9, f"Presets {preset_names[i]} and {preset_names[j]} produced too similar results"
...
class TestRealDataEdgeCases:
    ...
    def test_volatile_period_analysis(self):
        ...
        try:
            ...
            assert mean_certainty > 0.5, f"VIX regime assignments too uncertain: {mean_certainty}"
            
            # Should have observed multiple regimes
            states = model_results['most_likely_states']
            unique_states = len(np.unique(states))
            assert unique_states >= 2, f"VIX should show multiple regimes, only found {unique_states}"
            
        except Exception as e:
            if "network" in str(e).lower() or "connection" in str(e).lower() or "vix" in str(e).lower():
    ...
    def test_small_cap_vs_large_cap(self):
        ...
        try:
            ...
            large_cap_switches = np.sum(np.diff(large_cap_states) != 0) / len(large_cap_states)
            small_cap_switches = np.sum(np.diff(small_cap_states) != 0) / len(small_cap_states)
            ...
            assert 0.01 < large_cap_switches < 0.5, f"Large cap switching rate unrealistic: {large_cap_switches}"
            assert 0.01 < small_cap_switches < 0.5, f"Small cap switching rate unrealistic: {small_cap_switches}"
    ...
    def test_recent_data_availability(self):
        ...
        try:
            ...
            regime_probs = model_results['regime_probabilities']
            assert len(regime_probs) > 10, "Should have at least 10 trading days of data"
