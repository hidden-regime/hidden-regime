...
class TestPerformanceBenchmarks:
    ...
    def test_memory_efficiency_multiple_models(self, mock_ticker, large_mock_data, process_monitor):
        ...
        for i, ticker in enumerate(tickers):
            pipeline = hr.create_financial_pipeline(
                ticker,
                n_states=3,
                max_iterations=50  # Reduce iterations for speed
            )
    ...
    def test_convergence_speed(self, mock_ticker, large_mock_data):
        ...
        for max_iter in iteration_configs:
            ...
            model_output = pipeline.get_component_output('model')
            log_likelihood = getattr(model_output, 'log_likelihood', 0.0) if model_output else 0.0
        ...
        likelihoods = [convergence_results[k]['log_likelihood'] for k in iteration_configs]
        ...
        assert likelihood_improvement > -10.0, f"Likelihood degraded significantly: {likelihood_improvement}"
...
class TestScalabilityTests:
    ...
    def test_data_size_scalability(self, num_days):
        ...
        with patch('yfinance.Ticker') as mock_ticker_class:
            ...
            pipeline = hr.create_financial_pipeline(
                'VTI',  # Valid ticker
                n_states=3,
                model_config_overrides={'max_iterations': 50}  # Keep iterations constant
            )
            ...
            assert training_time < expected_max_time, f"Training time {training_time:.2f}s too slow for {num_days} days"
    ...
    def test_model_complexity_scalability(self, n_states):
        ...
        with patch('yfinance.download') as mock_download:
            ...
            assert training_time < expected_max_time, f"Training time {training_time:.2f}s too slow for {n_states} states"
