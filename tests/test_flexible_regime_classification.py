...
class TestFlexibleRegimeClassification:
    ...
    def test_threshold_based_classification(self):
        ...
        for return_pct, expected_regime in test_cases:
            ...
            assert regime == expected_regime, f"Return {return_pct:.1%} should be {expected_regime}, got {regime}"
    ...
    def test_flexible_state_mapping_no_forced_categories(self):
        ...
        emission_means_log = np.array([-0.00505901, 0.00921712, 0.02])  # Fixed realistic third state
        ...
        for state_idx, regime_name in mapping.items():
            ...
            assert abs(return_pct) <= 0.05, f"State {state_idx} ({regime_name}) has extreme return: {return_pct:.2%}"
    ...
    def test_variable_state_counts(self):
        ...
        emission_means_4 = np.array([-0.01, 0.001, 0.015, 0.08])  # Bear, Sideways, Bull, Euphoric
    ...
    def test_constrained_kmeans_prevents_extreme_centers(self):
        ...
        for i, mean_pct in enumerate(means_pct):
            assert abs(mean_pct) <= 0.08, f"State {i} has extreme constrained mean: {mean_pct:.2%}"
    ...
    def test_integration_with_pipeline(self):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            for name in regime_names:
                assert any(meaningful in name for meaningful in meaningful_names), f"Unexpected regime name: {name}"

            # Check that model has reasonable emission means
            if hasattr(model_output, 'emission_means_') and model_output.emission_means_ is not None:
                means_pct = np.exp(model_output.emission_means_) - 1

                # No extreme regimes
                for i, mean_pct in enumerate(means_pct):
                    assert abs(mean_pct) <= 0.08, f"State {i} has extreme emission mean: {mean_pct:.2%}"

                # States should be ordered by return
                assert np.all(np.diff(means_pct) >= -0.01), "States not properly ordered by return"
    ...
    def test_financial_domain_warnings(self):
        ...
        extreme_means = np.array([-0.001, 0.001, 0.15])  # Normal, Normal, Extreme (15% daily)
        ...
        inconsistent_means = np.array([0.02, -0.01, 0.001])  # Bull, Bear, Sideways (wrong order)
    ...
    def test_edge_cases(self):
        ...
        assert "Bull" in mapping_positive[2]  # Could be "Bull" or "Strong Bull" (3.56% < 5% threshold)
    ...
    def test_analysis_integration(self):
        ...
        if not bear_rows.empty:
            assert bear_rows['expected_return_pct'].iloc[0] < 0  # Bear should be negative

        if not bull_rows.empty:
            assert bull_rows['expected_return_pct'].iloc[0] > 0.01  # Bull should be meaningfully positive
