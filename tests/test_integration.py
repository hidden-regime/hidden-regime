...
class TestPipelineIntegration:
    ...
    def test_complete_single_stock_pipeline(self):
        ...
        with patch("hidden_regime.data.loader.yf.Ticker") as mock_ticker_class:
            ...
            assert (
                len(processed_data) >= len(raw_data) - 5
            )  # Some rows may be lost to edge effects
            ...
            processed_validation = self.validator.validate_data(
                processed_data, self.test_ticker
            )
    ...
    def test_multi_stock_pipeline_workflow(self):
        ...
        with patch("hidden_regime.data.loader.yf.Ticker") as mock_ticker_class:
            ...
            if (
                self.preprocessor.preprocessing_config.align_timestamps
                and len(processed_dict) > 1
            ):
                date_ranges = {}
    ...
    def test_configuration_interaction_workflow(self):
        ...
        with patch("hidden_regime.data.loader.yf.Ticker") as mock_ticker_class:
            ...
            if preprocessing_config.apply_smoothing:
                smoothed_cols = [
                    col for col in processed_data.columns if "smoothed" in col
                ]
    ...
    def test_error_propagation_through_pipeline(self):
        ...
        with warnings.catch_warnings():
            ...
            try:
                processed_corrupted = self.preprocessor.process_data(
...
class TestPipelineEdgeCases:
        ...
        minimal_data = pd.DataFrame(
            {
                ...
                ),  # Just above min_observations=30
    ...
    def test_single_stock_vs_multi_stock_consistency(self):
        ...
        with patch("hidden_regime.data.loader.yf.Ticker") as mock_ticker_class:
            ...
            single_data = self.loader.load_stock_data(
                ticker, "2024-01-01", "2024-03-31"
            )
            single_processed = self.preprocessor.process_data(single_data, ticker)

            # Load as part of multi-stock batch
            multi_data = self.loader.load_multiple_stocks(
                [ticker], "2024-01-01", "2024-03-31"
            )
