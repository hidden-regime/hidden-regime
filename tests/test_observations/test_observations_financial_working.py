...
class TestFinancialObservationGeneratorWorking:
    ...
    def test_preset_configurations(self):
        ...
        comprehensive_config = FinancialObservationConfig.create_comprehensive_financial()
    ...
    def test_macd_indicator(self):
        ...
        config = FinancialObservationConfig(generators=["macd"], normalize_features=False)
        generator = FinancialObservationGenerator(config)
        ...
        macd_data = observations[['macd_line', 'macd_signal', 'macd_histogram']].dropna()
        if len(macd_data) > 0:
    ...
    def test_bollinger_bands_indicator(self):
        ...
        config = FinancialObservationConfig(generators=["bollinger_bands"], normalize_features=False)
        generator = FinancialObservationGenerator(config)
    ...
    def test_volume_indicators_without_volume_data(self):
        ...
        data = pd.DataFrame({
            'close': 100 + np.random.normal(0, 1, 20)
            # No volume column
        }, index=dates)
    ...
    def test_normalization_functionality(self):
        ...
        config = FinancialObservationConfig(
            generators=["rsi", "volatility"],
            normalize_features=True
        )
        generator = FinancialObservationGenerator(config)
        
        dates = pd.date_range('2023-01-01', periods=300, freq='D')  # Long series for normalization
        ...
        if len(rsi_values) > 50:  # Need enough data for rolling normalization
            ...
            assert rsi_values.mean() != pytest.approx(50, abs=10)  # RSI normally centers around 50
    ...
    def test_error_handling_missing_volume_column(self):
        ...
        dates = pd.date_range('2023-01-01', periods=10, freq='D')
        data = pd.DataFrame({
            'close': np.random.randn(10)  # Has close but not volume
        }, index=dates)
    ...
    def test_get_observation_info_with_observations(self):
        ...
        dates = pd.date_range('2023-01-01', periods=30, freq='D')
        data = pd.DataFrame({
            'close': 100 + np.random.normal(0, 1, 30)
        }, index=dates)
    ...
    def test_average_price_generation(self):
        ...
        config = FinancialObservationConfig(generators=["average_price"], normalize_features=False)
        generator = FinancialObservationGenerator(config)
...
class TestFinancialObservationGeneratorCoverage:
    ...
    def test_various_price_column_configurations(self):
        ...
        dates = pd.date_range('2023-01-01', periods=20, freq='D')
        data = pd.DataFrame({
            'adj_close': 100 + np.random.normal(0, 1, 20)
        }, index=dates)
    ...
    def test_complex_generator_combinations(self):
        ...
        config = FinancialObservationConfig(
            generators=["log_return", "rsi", "macd", "bollinger_bands", "moving_average"],
            normalize_features=True
        )
        generator = FinancialObservationGenerator(config)
        ...
        expected_cols = [
            'log_return', 'rsi', 'macd_line', 'macd_signal', 'macd_histogram',
            'bb_upper', 'bb_middle', 'bb_lower', 'bb_position', 'sma_20'
        ]
