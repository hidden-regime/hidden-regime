...
class TestDataLoadingPerformance:
    ...
    def test_loading_time_scales_linearly(self):
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            for size in self.test_sizes:
                ...
                timing_results.append({
                    'size': len(data),
                    'time': load_time,
                    'time_per_row': load_time / len(data) if len(data) > 0 else float('inf')
                })
        ...
        for i in range(1, len(timing_results)):
            ...
            if prev_result['time_per_row'] > 0:
                ...
                assert time_ratio < 10.0, f"Performance degradation detected: {time_ratio:.2f}x slower per row"
    ...
    def test_caching_performance_improvement(self):
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            ...
            start_time = time.time()
            data1 = self.loader.load_stock_data("CACHE_TEST", "2024-01-01", "2024-12-31")
            first_load_time = time.time() - start_time
            
            # Second load (should use cache)
            start_time = time.time()
            data2 = self.loader.load_stock_data("CACHE_TEST", "2024-01-01", "2024-12-31")
            ...
            if first_load_time > 0.001:  # Only test if first load was measurable
                ...
                assert speedup_ratio > 2.0, f"Cache speedup insufficient: {speedup_ratio:.2f}x"
    ...
    def test_multi_stock_loading_efficiency(self):
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            ...
            assert time_ratio < 3.0, f"Batch loading too slow: {time_ratio:.2f}x individual time"
...
class TestValidationPerformance:
    ...
    def test_validation_scaling(self):
        ...
        for i in range(1, len(timing_results)):
            ...
            if prev_result['time_per_row'] > 0:
                time_ratio = curr_result['time_per_row'] / prev_result['time_per_row']
                assert time_ratio < 5.0, f"Validation scaling issue: {time_ratio:.2f}x slower per row"
    ...
    def test_outlier_detection_performance(self):
        ...
        for method, results in timing_results.items():
            assert results['time'] < 5.0, f"{method} method took too long: {results['time']:.2f}s"
    ...
    def test_validation_memory_usage(self):
        ...
        assert memory_retained < 50, f"Memory leak suspected: {memory_retained:.1f}MB retained"
...
class TestPreprocessingPerformance:
    ...
    def test_preprocessing_scaling(self):
        ...
        for i in range(1, len(timing_results)):
            ...
            if prev_result['time_per_row'] > 0:
                time_ratio = curr_result['time_per_row'] / prev_result['time_per_row']
                assert time_ratio < 3.0, f"Preprocessing scaling issue: {time_ratio:.2f}x slower per row"
    ...
    def test_feature_engineering_performance(self):
        ...
        for config_name, config in configs.items():
            ...
            start_time = time.time()
            processed_data = preprocessor.process_data(test_data.copy(), f"FEATURE_{config_name}")
            processing_time = time.time() - start_time
        ...
        for config_name, results in timing_results.items():
            assert results['time'] < 10.0, f"{config_name} config too slow: {results['time']:.2f}s"
        ...
        if timing_results['minimal']['time'] > 0:
            overhead_ratio = timing_results['full']['time'] / timing_results['minimal']['time']
            assert overhead_ratio < 5.0, f"Feature engineering overhead too high: {overhead_ratio:.2f}x"
    ...
    def test_multi_series_processing_performance(self):
        ...
        if individual_time > 0:
            time_ratio = batch_time / individual_time
            assert time_ratio < 3.0, f"Batch processing too slow: {time_ratio:.2f}x individual time"
...
class TestEndToEndPerformance:
    ...
    def test_complete_pipeline_performance(self):
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            ...
            # Step 1: Load
            raw_data = self.loader.load_stock_data("PIPELINE_PERF", "2024-01-01", "2024-12-31")
            load_time = time.time()
            ...
            # Step 4: Re-validate
            final_validation = self.validator.validate_data(processed_data, "PIPELINE_PERF")
            ...
            assert stage_times['total'] < 30.0, f"Pipeline too slow: {stage_times['total']:.2f}s total"
            
            # No single stage should dominate excessively
            max_stage_time = max(stage_times[stage] for stage in ['loading', 'validation', 'processing'])
            assert max_stage_time < stage_times['total'] * 0.8, "One stage taking too much time"
    ...
    def test_pipeline_memory_efficiency(self):
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            ...
            for i in range(5):
                ...
                validation = self.validator.validate_data(raw_data, f"MEMORY_PIPELINE_{i}")
                processed_data = self.preprocessor.process_data(raw_data, f"MEMORY_PIPELINE_{i}")
                final_validation = self.validator.validate_data(processed_data, f"MEMORY_PIPELINE_{i}")
            ...
            assert memory_increase < 200, f"Possible memory leak: {memory_increase:.1f}MB increase"
...
class BenchmarkResults:
    ...
    def run_standard_benchmarks() -> Dict[str, float]:
        ...
        with patch('hidden_regime.data.loader.yf.Ticker') as mock_ticker_class:
            ...
            raw = loader.load_stock_data("PIPELINE_BENCHMARK", "2024-01-01", "2024-06-30")
