...
def create_sample_stock_data(
    ...
    if add_outliers:
        ...
        outlier_magnitudes = np.random.choice([-1, 1], n_outliers) * np.random.uniform(0.1, 0.2, n_outliers)
    ...
    if add_missing and missing_pct > 0:
        ...
        for idx in missing_indices:
            cols_to_missing = np.random.choice(['price', 'log_return'], 
...
def create_invalid_stock_data() -> Dict[str, pd.DataFrame]:
    ...
    scenarios['duplicate_dates'].loc[5, 'date'] = scenarios['duplicate_dates'].loc[4, 'date']
    
    return scenarios


def create_multi_stock_data(
    tickers: list = ['AAPL', 'GOOGL', 'MSFT'],
    n_days: int = 50
...
class MockYFinanceTicker:
    ...
    def history(self, start, end, auto_adjust=True, prepost=False):
        ...
        base_price = {"AAPL": 150, "GOOGL": 2500, "MSFT": 300, "SPY": 400}.get(self.ticker, 100)
