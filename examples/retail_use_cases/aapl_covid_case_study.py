...
class AAPLCOVIDCaseStudy:
    ...
    def run_online_trading_simulation(self, data):
        ...
        for i, (date, row) in enumerate(trading_data.iterrows()):
            ...
            regime_str = regime_info.get('regime_name', f"State {regime_info['regime']}")
            ...
            self.results['regime_probs'].append(regime_info.get('state_probabilities', [0.33, 0.33, 0.34]))
            ...
            if (i + 1) % 50 == 0 or (i + 1) == len(trading_data):
                ...
                current_regime = regime_info.get('regime_name', f"State {regime_info['regime']}")
                print(f"   📈 Day {i+1:3d} ({progress:5.1f}%): {date.strftime('%Y-%m-%d')} | "
    ...
    def _get_position_recommendation(self, regime_info):
        ...
        return np.clip(confidence_adjusted, -self.max_position_size, self.max_position_size)
    ...
    def analyze_performance(self, simulator):
        ...
        aapl_start_price = results_df.iloc[0]['portfolio_value'] / results_df.iloc[0]['position'] if results_df.iloc[0]['position'] != 0 else 100  # Approximation
        aapl_end_price = results_df.iloc[-1]['portfolio_value'] / results_df.iloc[-1]['position'] if results_df.iloc[-1]['position'] != 0 else 100
        buy_hold_return = (aapl_end_price - aapl_start_price) / aapl_start_price if aapl_start_price != 0 else 0
    ...
    def _analyze_key_events(self, results_df):
        ...
        for event_name, start_date, end_date in key_events:
            ...
            if not event_data.empty:
                ...
                print(f"   {event_name:15s}: {event_return:+7.1%} | "
    ...
    def create_visualizations(self, results_df):
        ...
        fig.suptitle(f'{self.ticker} COVID-19 Case Study: Online Learning Performance', fontsize=16, fontweight='bold')
        ...
        ax1.plot(results_df['date'], cumulative_aapl, label='AAPL Buy & Hold', linewidth=2, color='orange')
        ...
        position_colors = ['red' if p < 0 else 'green' if p > 0.5 else 'orange' for p in results_df['position']]
        ax3.scatter(results_df['date'], results_df['position'], c=position_colors, alpha=0.6, s=10)
        ...
        ax4.plot(results_df['date'], results_df['confidence'], color='purple', alpha=0.7)
        ax4.fill_between(results_df['date'], results_df['confidence'], alpha=0.3, color='purple')
        ...
        plt.savefig(viz_path / 'aapl_covid_case_study.png', dpi=300, bbox_inches='tight')
    ...
    def generate_trading_insights(self, results_df):
        ...
        crisis_periods = results_df[results_df['regime_name'].str.contains('bear|crisis', case=False)]
        if not crisis_periods.empty:
            ...
            print(f"   Days before market bottom (approx): {(pd.Timestamp('2020-03-23') - first_crisis).days}")
        ...
        print("   1. Monitor regime confidence - reduce positions when confidence < 60%")
        ...
        print("   4. Position sizing based on regime confidence improves risk-adjusted returns")
        print("   5. Short positions during crisis regimes provided downside protection")
