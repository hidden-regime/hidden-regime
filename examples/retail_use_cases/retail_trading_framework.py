...
class RetailTradingSimulator:
    ...
    def execute_trade(self, target_position: float, current_price: float, 
                     timestamp: datetime, regime: str, confidence: float) -> Dict[str, Any]:
        """
        ...
        """
        # Check trading rules
        if self.days_in_position < self.config.regime_hold_days and len(self.trade_history) > 0:
        ...
        if trade_value < total_costs * 10:  # Don't trade if costs > 10% of trade
            trade_details['reason'] = 'Transaction costs too high relative to trade size'
            return trade_details
    ...
    def update_portfolio(self, current_price: float, timestamp: datetime) -> Dict[str, Any]:
        """
        ...
        portfolio_status = {
            ...
            'total_return': (total_capital - self.config.initial_capital) / self.config.initial_capital,
...
class PerformanceAnalyzer:
    ...
    def analyze_performance(self, performance_history: List[Dict], 
        ...
        avg_trade_return = self._calculate_avg_trade_return(trade_history) if num_trades > 0 else 0
        ...
        if benchmark_returns is not None and len(benchmark_returns) == len(returns):
            ...
            metrics.update({
                ...
                'information_ratio': (annualized_return - benchmark_total_return * (252/days_traded)) / 
...
class RegimeTrader:
    ...
    def process_new_observation(self, new_return: float, current_price: float, 
        ...
        update_result = self.hmm_model.add_observation(new_return, update_parameters=True)
        ...
        # Update portfolio
        portfolio_status = self.trading_simulator.update_portfolio(current_price, timestamp)
    ...
    def get_performance_summary(self, benchmark_returns: np.ndarray = None) -> Dict[str, Any]:
        ...
        if len(regime_df) > 0:
            ...
            performance_metrics.update({
                ...
                'total_regime_changes': len(regime_df) - 1 if len(regime_df) > 1 else 0
            })
...
def load_market_data_for_trading(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
...
def compare_regime_models(data: pd.DataFrame, regime_types: List[str] = None) -> Dict[str, Any]:
    """
    ...
    for regime_type in regime_types:
        try:
            ...
            trader.train_initial_model(training_data['log_return'].values, verbose=False)
