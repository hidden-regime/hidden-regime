...
from retail_trading_framework import RegimeTrader, RetailTradingSimulator, TradingConfig
...
class DOGEExplosiveGrowthCaseStudy:
    ...
    def load_data(self):
        ...
        try:
            ...
            print(
                f"   💰 Price range: ${data['Close'].min():.4f} to ${data['Close'].max():.2f}"
            )
    ...
    def prepare_training_data(self, data):
        ...
        print(
            f"   🎯 Sharpe ratio: {returns.mean() / returns.std() * np.sqrt(365):.2f}"
        )
    ...
    def run_online_trading_simulation(self, data):
        ...
        for i, (date, row) in enumerate(trading_data.iterrows()):
            ...
            regime_str = regime_info.get(
                "regime_name", f"State {regime_info.get('regime', 0)}"
            )
            ...
            self.results["regime_probs"].append(
                regime_info.get("state_probabilities", [0.25, 0.25, 0.25, 0.25])
            )
            ...
            if (
                (i + 1) % 30 == 0
                or (i + 1) == len(trading_data)
                or current_return > 0.5
                or current_return < -0.5
            ):
                ...
                print(
                    f"   {price_change} Day {i+1:3d} ({progress:5.1f}%): {date.strftime('%Y-%m-%d')} | "
    ...
    def _get_crypto_position_recommendation(
        self, regime_info, volatility, current_price
    ):
    ...
    def _analyze_crypto_key_events(self, results_df):
        ...
        for event_name, start_date, end_date in key_events:
            ...
            if not event_data.empty:
                ...
                dominant_regime = (
                    event_data["regime_name"].mode().iloc[0]
                    if not event_data["regime_name"].mode().empty
                    else "Unknown"
                )
                ...
                print(
                    f"   {event_name:15s}: Portfolio {event_return:+7.1%} | DOGE {price_return:+7.1%} | "
    ...
    def create_crypto_visualizations(self, results_df):
        ...
        fig.suptitle(
            f"{self.ticker} Explosive Growth Case Study: Online Learning Performance",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i, (date, regime) in enumerate(
            zip(results_df["date"], results_df["regime_name"])
        ):
        ...
        portfolio_drawdown = (
            portfolio_cumulative - portfolio_rolling_max
        ...
        plt.savefig(
            viz_path / "doge_explosive_growth_case_study.png",
            dpi=300,
            bbox_inches="tight",
        )
    ...
    def generate_crypto_insights(self, results_df):
        ...
        # Volatility impact
        high_vol_days = results_df[
            results_df["volatility"] > results_df["volatility"].quantile(0.9)
        ]
        if not high_vol_days.empty:
            avg_position_high_vol = high_vol_days["position"].mean()
            avg_position_low_vol = results_df[
                results_df["volatility"] <= results_df["volatility"].quantile(0.9)
            ]["position"].mean()

            print(f"\n📊 Volatility Impact:")
            print(
                f"   Avg position during high volatility: {avg_position_high_vol:.1%}"
            )
            print(
                f"   Avg position during normal volatility: {avg_position_low_vol:.1%}"
            )
        ...
        print(
            "   2. Increase adaptation rate for faster regime detection in volatile crypto"
        )
        print(
            "   3. Implement volatility-based position sizing to manage extreme moves"
        )
