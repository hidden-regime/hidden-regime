...
from hidden_regime.visualization.plotting import (
    setup_financial_plot_style, REGIME_COLORS, get_regime_colors
)
...
class RetailPerformanceAnalyzer:
    ...
    def create_performance_dashboard(self, 
        ...
        # Add title
        fig.suptitle('Hidden Regime Trading Strategy - Performance Dashboard', 
    ...
    def _plot_portfolio_value(self, ax, perf_df: pd.DataFrame, benchmark_data: pd.DataFrame = None):
        ...
        ax.plot(dates, portfolio_pct, linewidth=2.5, label='Hidden Regime Strategy', 
        ...
        if benchmark_data is not None and 'price' in benchmark_data.columns:
            ...
            benchmark_pct = (np.array(aligned_benchmark) / aligned_benchmark[0] - 1) * 100
            ax.plot(dates, benchmark_pct, linewidth=2, label='Buy & Hold', 
                   color='#CD853F', alpha=0.7, linestyle='--')
    ...
    def _plot_performance_summary(self, ax, perf_df: pd.DataFrame, benchmark_data: pd.DataFrame = None):
...
Peak Capital: ${perf_df['total_capital'].max():,.0f}"""
    ...
    def _plot_regime_timeline(self, ax, regime_df: pd.DataFrame, perf_df: pd.DataFrame):
        ...
        if len(regime_df) == 0:
            ax.text(0.5, 0.5, 'No regime data available', transform=ax.transAxes, 
                   ha='center', va='center')
            return
        ...
        for i in range(len(dates) - 1):
            ...
            ax.axvspan(dates[i], dates[i + 1], 
                      ...
                      alpha=alpha, label=regime if i == 0 or regimes[i] != regimes[i-1] else "")
        
        # Plot portfolio returns on secondary axis
        ax2 = ax.twinx()
        portfolio_returns = perf_df['daily_return'].values * 100  # Convert to percentage
        ...
        handles = [plt.Rectangle((0,0),1,1, color=regime_colors.get(regime, '#808080'), 
                               alpha=0.7) for regime in unique_regimes]
    ...
    def _plot_returns_distribution(self, ax, perf_df: pd.DataFrame):
        ...
        # Create histogram
        ax.hist(returns, bins=30, density=True, alpha=0.7, color='lightblue', 
        ...
        # Add statistics
        ax.axvline(mu, color='blue', linestyle='--', alpha=0.8, label=f'Mean: {mu:.2f}%')
        ax.axvline(mu + 2*sigma, color='red', linestyle=':', alpha=0.8, label=f'+2σ: {mu + 2*sigma:.2f}%')
        ax.axvline(mu - 2*sigma, color='red', linestyle=':', alpha=0.8, label=f'-2σ: {mu - 2*sigma:.2f}%')
    ...
    def _plot_drawdown_analysis(self, ax, perf_df: pd.DataFrame):
        ...
        max_dd_date = dates.iloc[max_dd_idx] if hasattr(dates, 'iloc') else dates[max_dd_idx]
        max_dd_value = drawdowns[max_dd_idx]
        
        ax.scatter([max_dd_date], [-max_dd_value], color='red', s=100, zorder=5, 
    ...
    def _plot_trading_activity(self, ax, trade_history: List[Dict], regime_df: pd.DataFrame):
        """Plot trading activity by regime."""
        
        if not trade_history:
            ax.text(0.5, 0.5, 'No trading activity', transform=ax.transAxes, 
                   ha='center', va='center')
            return
        ...
        if not regime_trades:
            ax.text(0.5, 0.5, 'No executed trades', transform=ax.transAxes, 
                   ha='center', va='center')
            return
    ...
    def _plot_regime_performance(self, ax, perf_df: pd.DataFrame, regime_df: pd.DataFrame):
        ...
        for _, perf_row in perf_df_copy.iterrows():
            ...
            time_diffs = np.abs((regime_df_copy['timestamp'] - perf_row['timestamp']).dt.total_seconds())
            closest_regime_idx = np.argmin(time_diffs)
            
            regime_info = regime_df_copy.iloc[closest_regime_idx]
            aligned_data.append({
                'regime': regime_info['regime'],
                'daily_return': perf_row['daily_return'] * 100  # Convert to percentage
            })
        
        aligned_df = pd.DataFrame(aligned_data)
        
        # Calculate average returns by regime
        regime_returns = aligned_df.groupby('regime')['daily_return'].agg(['mean', 'std', 'count']).reset_index()
        
        if len(regime_returns) == 0:
            ax.text(0.5, 0.5, 'No regime performance data', transform=ax.transAxes, 
                   ha='center', va='center')
            return
        ...
        bars = ax.bar(regimes, mean_returns, color=colors, alpha=0.7, 
                     yerr=std_returns, capsize=5, edgecolor='black')
    ...
    def _plot_rolling_metrics(self, ax, perf_df: pd.DataFrame):
        ...
        ax.plot(rolling_dates, rolling_sharpe, color='blue', linewidth=2, 
        ...
        ax2 = ax.twinx()
        ax2.plot(rolling_dates, rolling_volatility, color='red', linewidth=2, 
                label=f'{window}-Day Rolling Volatility')
        ax2.set_ylabel('Volatility (%)', color='red')
        ax2.tick_params(axis='y', labelcolor='red')
        
        ax.set_title(f'Rolling Performance Metrics ({window}-Day Window)', fontweight='bold')
        ax.set_xlabel('Date')
        ax.grid(True, alpha=0.3)
    ...
    def create_regime_transition_analysis(self, regime_history: List[Dict], 
        ...
        for transition in transitions:
            ...
            if len(after_transition) >= 5:  # Need at least 5 days after transition
                ...
                performance = {
                    ...
                    'confidence_change': transition['confidence_after'] - transition['confidence_before']
                }
        ...
        return {
            ...
            'avg_confidence_change': np.mean([t['confidence_change'] for t in transitions]) if transitions else 0
        }
    
    def generate_retail_trader_report(self, 
                                    performance_history: List[Dict],
                                    regime_history: List[Dict],
                                    trade_history: List[Dict],
                                    benchmark_data: pd.DataFrame = None,
                                    asset_name: str = "Asset") -> str:
        """
        ...
        if regime_history:
            ...
            for regime, pct in regime_dist.items():
                regime_analysis += f"<li><strong>{regime}</strong>: {pct:.1%} of time</li>"
            regime_analysis += "</ul>"
        ...
        if trade_history:
            ...
            trading_summary = f"<p><strong>Total Trades:</strong> {len(executed_trades)}</p>"
...
def calculate_benchmark_comparison(strategy_returns: np.ndarray, 
