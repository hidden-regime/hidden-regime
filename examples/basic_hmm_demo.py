...
def basic_hmm_training_demo():
    ...
    print(
        f"   Return statistics: mean={np.mean(returns):.4f}, std={np.std(returns):.4f}"
    )
    ...
    print(
        f"   ✓ Final log-likelihood: {hmm.training_history_['final_log_likelihood']:.2f}"
    )
    ...
    if standardization_applied:
        print(
            f"   ✓ State standardization applied with confidence: {hmm._standardization_confidence:.2f}"
        )
    ...
    for i in range(3):
        ...
        if (
            standardization_applied
            and hasattr(hmm, "_state_mapping")
            and hmm._state_mapping
        ):
            config_obj = hmm._state_standardizer.standard_configs["3_state"]
            mapped_state = hmm._state_mapping.get(i, i)
            if (
                isinstance(mapped_state, int)
                and config_obj
                and mapped_state < len(config_obj.state_names)
            ):
    ...
    for state in range(3):
        ...
        if state_mask.any():
            ...
            print(
                f"   Detected State {state}: frequency={frequency:.1%}, "
...
def real_time_regime_tracking_demo(hmm, test_returns):
    ...
    for i, new_return in enumerate(test_returns[:20]):  # Show first 20 updates
        ...
        if i < 10 or i % 5 == 0:  # Show first 10, then every 5th
            print(
                f"   Day {i+1:2d}: Return={new_return:7.4f} | "
        ...
        if standardization_applied:
            ...
            if config_obj and hasattr(hmm, "_state_mapping"):
                ...
                for i in range(hmm.n_states):
                    mean, std = hmm.emission_params_[i]
                    mapped_state = hmm._state_mapping.get(i, i)
                    if isinstance(mapped_state, int) and mapped_state < len(
                        config_obj.state_names
                    ):
                        regime_name = config_obj.state_names[mapped_state]
                    else:
                        regime_name = f"State {i}"
...
def convenience_functions_demo():
    ...
    detected_states = hr.detect_regimes(
        returns, n_states=3, max_iterations=50, verbose=False
    )
    ...
    analysis = hr.analyze_regime_transitions(
        returns, dates, n_states=3, max_iterations=50, verbose=False
    )
...
def plotting_demo(hmm, returns, predicted_states, true_states, dates):
    ...
    try:
        ...
        plt.suptitle(
            "HMM Regime Analysis - Comprehensive View",
            fontsize=16,
            fontweight="bold",
            y=0.96,
        )
        ...
        fig = standardizer.plot(
            emission_params, plot_type="validation", figsize=(10, 6)
        )
        plt.show()
        ...
        for state_idx, regime_name in state_mapping.items():
            ...
            volatility = (
                emission_params[state_idx, 1] * np.sqrt(252) * 100
            )  # Annualized %
            print(
                f"     {regime_name}: {mean_ret:+.1f}% return, {volatility:.1f}% volatility"
            )
        ...
        if true_states is not None and len(true_states) == len(predicted_states):
            ...
            ax.plot(
                ...
                predicted_states,
                "s-",
                label="Predicted Regimes",
                alpha=0.7,
                markersize=3,
            )

            ax.set_title(
                f"Regime Detection Accuracy: {accuracy:.1%}", fontweight="bold"
            )
        ...
        print(
            "   • Individual plot types: 'regimes', 'transitions', 'probabilities', etc."
        )
        print(
            "   • Add save_path parameter to save plots: hmm.plot(..., save_path='my_plot.png')"
        )
        ...
        axes[0].scatter(
            range(len(returns)), returns, c=true_states, cmap="viridis", alpha=0.7, s=20
        )
        ...
        for state in range(3):
            axes[2].plot(
                ...
                state_probs[:, state],
                color=colors[state],
                label=f"State {state} ({regime_names[state]})",
                alpha=0.8,
            )
...
def main():
    ...
    try:
        ...
        hmm, returns, predicted_states, state_probs, true_states = (
            basic_hmm_training_demo()
        )
        ...
        print(
            "If you see this error, there may be an issue with the HMM implementation."
        )
        raise
