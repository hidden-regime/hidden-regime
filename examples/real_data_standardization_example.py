...
def load_sample_market_data():
    ...
    market_periods = [
        ...
        {'start': '2007-01-01', 'end': '2007-09-30', 'regime': 'bull', 
         'mean': 0.0008, 'std': 0.012, 'description': 'Pre-Crisis Bull'},
        
        # 2008 Financial Crisis (Oct 2007 - Mar 2009)
        {'start': '2007-10-01', 'end': '2009-03-31', 'regime': 'crisis',
         'mean': -0.0015, 'std': 0.035, 'description': '2008 Financial Crisis'},
        
        # Recovery Period (Apr 2009 - Dec 2010)
        {'start': '2009-04-01', 'end': '2010-12-31', 'regime': 'bull',
         'mean': 0.0012, 'std': 0.018, 'description': 'Post-Crisis Recovery'},
        
        # European Debt Crisis (Jan 2011 - Jun 2012)
        {'start': '2011-01-01', 'end': '2012-06-30', 'regime': 'bear',
         'mean': -0.0005, 'std': 0.022, 'description': 'European Debt Crisis'},
        
        # Long Bull Market (Jul 2012 - Jan 2020)
        {'start': '2012-07-01', 'end': '2020-01-31', 'regime': 'bull',
         'mean': 0.0006, 'std': 0.014, 'description': 'Long Bull Market'},
        
        # COVID Crash (Feb 2020 - Apr 2020)
        {'start': '2020-02-01', 'end': '2020-04-30', 'regime': 'crisis',
         'mean': -0.0025, 'std': 0.045, 'description': 'COVID-19 Crash'},
        
        # COVID Recovery (May 2020 - Dec 2023)
        {'start': '2020-05-01', 'end': '2023-12-31', 'regime': 'bull',
         'mean': 0.0008, 'std': 0.016, 'description': 'COVID Recovery'}
    ]
    ...
    print(f"   Return statistics: mean={market_data['log_return'].mean():.4f}, std={market_data['log_return'].std():.4f}")
...
def demonstrate_standardization_selection():
    ...
    for regime_type, description in approaches:
        ...
        results[regime_type] = {
            ...
            'standardization_confidence': hmm._standardization_confidence if standardization_applied else 0,
...
def validate_detected_regimes(hmm, market_data):
    ...
    print(f"   Mean ordering correct: {'✅' if validation_details['mean_ordering_correct'] else '❌'}")
    print(f"   Volatility reasonable: {'✅' if validation_details['volatility_reasonable'] else '❌'}")
    ...
    for period in unique_periods:
        ...
        if hasattr(hmm, '_state_standardizer') and hmm._state_standardizer is not None:
            ...
            if config_obj and hasattr(hmm, '_state_mapping') and hmm._state_mapping:
                ...
                mapped_value = hmm._state_mapping.get(dominant_state, hmm._state_mapping.get(np.int64(dominant_state), dominant_state))
        ...
        print(f"     Predicted: {standardized_name} (confidence: {dominant_confidence:.2f}) {match_status}")
...
def perform_regime_based_analysis(hmm, market_data, predicted_states):
    ...
    print(f"   Standardization applied: {'✅' if model_info.get('standardization_applied', False) else '❌'}")
    
    if 'standardization_confidence' in analysis:
        print(f"   Standardization confidence: {analysis['standardization_confidence']:.3f}")
    ...
    for state in range(hmm.n_states):
        if state in regime_stats:
            ...
            print(f"     Mean return: {stats['mean_return']:.4f} ({stats['mean_return']*252:.1%} annualized)")
            print(f"     Volatility: {stats['std_return']:.4f} ({stats['std_return']*np.sqrt(252):.1%} annualized)")
...
def demonstrate_real_time_detection(hmm, market_data):
    ...
    covid_mask = (market_data['date'] >= '2020-02-01') & (market_data['date'] <= '2020-05-31')
    ...
    for i, (date, return_val) in enumerate(zip(covid_dates, covid_returns)):
        ...
        if i > 0 and regime_info['most_likely_regime'] != regime_history[i-1]['most_likely_regime']:
        ...
        if i < 10 or i % 20 == 0 or return_val < -0.05 or return_val > 0.05:
            ...
            print(f"   {date_str}: Return={return_val:7.4f} | "
    ...
    for change in regime_changes:
        ...
        print(f"     {date_str} (Day {change['day']}): {change['from_regime']} → {change['to_regime']}")
        print(f"       Trigger return: {change['return']:.4f}, Confidence: {change['confidence']:.3f}")
...
def main():
    ...
    try:
        ...
        transition_analysis = analyze_regime_dynamics(best_hmm, market_data, predicted_states)
        
        # 4. Perform comprehensive regime-based analysis
        comprehensive_analysis = perform_regime_based_analysis(best_hmm, market_data, predicted_states)
        ...
        print(f"\nKey Findings:")
        print(f"- Best configuration: {best_hmm.config.regime_type} with {best_hmm.n_states} states")
        print(f"- Standardization confidence: {best_hmm._standardization_confidence:.3f}")
        print(f"- Model log-likelihood: {best_hmm.score(market_data['log_return'].values):.2f}")
        ...
        print(f"• Automatic regime configuration selection based on data characteristics")
        ...
        print(f"• Integration of standardization framework with practical trading applications")
