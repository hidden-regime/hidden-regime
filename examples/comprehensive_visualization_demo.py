...
def main():
    ...
    try:
        ...
        plt.savefig(f"{output_dir}/01_dataloader_comprehensive.png", dpi=150, bbox_inches='tight')
        plt.show()
        ...
        for i, plot_type in enumerate(plot_types, 2):
            ...
            try:
                ...
                plt.savefig(f"{output_dir}/01_{i}_dataloader_{plot_type}.png", dpi=150, bbox_inches='tight')
                plt.show()
        ...
        fig = loader.plot(data, plot_type='all', figsize=(16, 12))
        plt.savefig(f"{output_dir}/01_synthetic_comprehensive.png", dpi=150, bbox_inches='tight')
        plt.show()
    ...
    hmm_plot_types = ['regimes', 'probabilities', 'transitions', 'statistics', 'convergence', 'duration']
    for i, plot_type in enumerate(hmm_plot_types, 2):
        ...
        try:
            ...
            plt.savefig(f"{output_dir}/02_{i}_hmm_{plot_type}.png", dpi=150, bbox_inches='tight')
            plt.show()
    ...
    for state_idx, regime_name in state_mapping.items():
        ...
        print(f"   State {state_idx} → {regime_name}: μ={mean_ret:.4f}, σ={volatility:.4f}")
    print()
    ...
    try:
        ...
        plt.savefig(f"{output_dir}/03_standardizer_comprehensive.png", dpi=150, bbox_inches='tight')
        plt.show()
    ...
    standardizer_plot_types = ['characteristics', 'validation', 'comparison', 'economic']
    for i, plot_type in enumerate(standardizer_plot_types, 2):
        print(f"3.{i} Creating StateStandardizer {plot_type} visualization...")
        try:
            fig = standardizer.plot(emission_params, plot_type=plot_type, figsize=(10, 8))
            plt.savefig(f"{output_dir}/03_{i}_standardizer_{plot_type}.png", dpi=150, bbox_inches='tight')
            plt.show()
    ...
    for config_name in regime_configs:
        print(f"4.{regime_configs.index(config_name)+1} Training {config_name} model and visualizing...")
        ...
        try:
            ...
            fig = hmm_multi.plot(returns, dates=dates, plot_type='regimes', figsize=(14, 6))
            plt.suptitle(f'{config_name.replace("_", "-").title()} Regime Classification', fontsize=14, fontweight='bold')
            plt.savefig(f"{output_dir}/04_{regime_configs.index(config_name)+1}_{config_name}_regimes.png", 
                       dpi=150, bbox_inches='tight')
            plt.show()
            
            # Create characteristics heatmap
            emission_multi = hmm_multi.emission_params_
            fig = standardizer_multi.plot(emission_multi, plot_type='characteristics', figsize=(10, 8))
            plt.savefig(f"{output_dir}/04_{regime_configs.index(config_name)+1}_{config_name}_characteristics.png", 
                       dpi=150, bbox_inches='tight')
            plt.show()
    ...
    from hidden_regime.visualization.plotting import get_regime_colors, setup_financial_plot_style
    ...
    bars = ax1.barh(y_pos, [1]*len(regime_names), 
                    color=[colors[name] for name in regime_names])
    ...
    print("   • DataLoader: Market data analysis (price, returns, distributions, volume)")
