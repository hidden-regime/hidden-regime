...
class RegimeBasedStrategy:
    ...
    def train_model(self, returns: np.ndarray, verbose: bool = False) -> None:
        ...
        if verbose:
            print(f"Model trained: {self.hmm_model.n_states} states, "
    ...
    def update_performance(self, current_price: float, date: datetime) -> None:
        ...
        performance_record = {
            ...
            'return': (portfolio_value / self.portfolio_value - 1) if self.portfolio_value > 0 else 0.0
        }
    ...
    def _calculate_win_rate(self) -> float:
        ...
        for i in range(1, len(self.trades)):
            if self.trades[i]['dollar_amount'] * (self.trades[i]['price'] - self.trades[i-1]['price']) > 0:
                profitable_trades += 1
        
        return profitable_trades / max(1, len(self.trades) - 1)


def generate_market_data_with_regimes(n_days: int = 252, random_seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(n_days):
        ...
        daily_return = np.random.normal(regime['mean'] / 252, regime['std'] / np.sqrt(252))
...
def run_strategy_backtest(
    ...
    if verbose:
        print(f"Training period: {train_data['date'].iloc[0].date()} to {train_data['date'].iloc[-1].date()}")
    ...
    for idx, row in test_data.iterrows():
        ...
        if verbose and day_count % 50 == 0:
            ...
            print(f"  Day {day_count}: Return = {current_return:.2%}, "
...
def analyze_strategy_performance(
    ...
    benchmark_return = (benchmark_data['price'].iloc[-1] / benchmark_data['price'].iloc[0]) - 1
    benchmark_volatility = benchmark_data['log_return'].std() * np.sqrt(252)
    benchmark_sharpe = (benchmark_return * 252 / len(benchmark_data)) / benchmark_volatility
    ...
    if strategy.trades:
        ...
        for regime, trades in trades_by_regime.items():
            ...
            print(f"  {regime_names.get(regime, f'Regime {regime}')}: {len(trades)} trades, "
...
def plot_strategy_results(
    ...
    try:
        ...
        benchmark_normalized = (benchmark_data['price'] / benchmark_data['price'].iloc[0] * 
                              performance_df['portfolio_value'].iloc[0])
        axes[0, 0].plot(benchmark_data['date'], benchmark_normalized, 
        ...
        axes[1, 0].hist(benchmark_returns, bins=30, alpha=0.7, label='Benchmark', density=True)
        ...
        if strategy.trades:
            ...
            for regime in [0, 1, 2]:
                regime_trades = [(date, amount) for date, amount, r in zip(trade_dates, trade_amounts, trade_regimes) if r == regime]
                if regime_trades:
                    dates, amounts = zip(*regime_trades)
                    axes[1, 1].scatter(dates, amounts, c=colors[regime], 
        ...
        try:
            plt.savefig('hmm_trading_strategy_results.png', dpi=150, bbox_inches='tight')
...
def main():
    ...
    try:
        ...
        print(f"   Return statistics: mean={data['log_return'].mean():.4f}, "
              f"std={data['log_return'].std():.4f}")
        
        # Run backtest
        print("\n2. Running regime-based trading strategy...")
        strategy, performance_df = run_strategy_backtest(data, train_pct=0.6, verbose=True)
