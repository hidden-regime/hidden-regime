    ...
    def _check_quality_alerts(self, ticker, current_record):
        ...
        if len(history) >= self.alert_thresholds["consecutive_failures"]:
            recent_qualities = [
                ...
                for record in history[-self.alert_thresholds["consecutive_failures"] :]
            ]

            if all(
                q < self.alert_thresholds["quality_score_min"] for q in recent_qualities
            ):
                alerts.append(
                    {
    ...
    def generate_quality_report(self, tickers=None):
        ...
        if all_current_qualities:
            report["summary"] = {
                ...
                "tickers_below_threshold": sum(
                    1
                    for q in all_current_qualities
        ...
        if avg_quality < 0.5:
            report["recommendations"].append(
                "Critical: Portfolio-wide data quality issues detected. Review data sources and collection processes."
            )
        elif avg_quality < 0.7:
            report["recommendations"].append(
                "Warning: Below-average data quality. Consider implementing data cleaning procedures."
            )
        else:
            report["recommendations"].append(
                "Good: Data quality within acceptable ranges. Continue monitoring."
            )

        if report["summary"].get("tickers_below_threshold", 0) > 0:
            report["recommendations"].append(
                f"Action needed: {report['summary']['tickers_below_threshold']} tickers below quality threshold. Investigate specific data issues."
            )
...
def simulate_quality_monitoring(monitor, tickers, n_periods=10):
    ...
    print(
        f"Simulating quality monitoring for {len(tickers)} stocks over {n_periods} periods..."
    )
    ...
    for period in range(n_periods):
        ...
        for ticker in tickers:
            ...
            missing_pct = max(
                0, np.random.normal(0.02 * (1 - base_quality_factor), 0.01)
            )
            n_missing = int(data_size * missing_pct)

            if n_missing > 0:
                missing_indices = np.random.choice(
                    data_size, min(n_missing, data_size // 2), replace=False
                )
            ...
            if np.random.random() > base_quality_factor:
                outlier_indices = np.random.choice(
                    data_size, max(1, data_size // 20), replace=False
                )
                prices[outlier_indices] *= np.random.choice(
                    [0.5, 1.5], len(outlier_indices)
                )
            ...
            # Monitor quality
            validation_result, alerts = monitor.monitor_stock_quality(
                ticker, mock_data, current_date
            )

            print(
                f"  {ticker}: Quality {validation_result.quality_score:.3f} "
        ...
        if not period_alerts:
            ...
            high_alerts = sum(
                1 for alert in period_alerts if alert["severity"] == "HIGH"
            )
            med_alerts = len(period_alerts) - high_alerts
            print(
                f"  🚨 Period alerts: {high_alerts} high, {med_alerts} medium severity"
            )
...
def demonstrate_quality_reporting(monitor, tickers):
    ...
    print(
        f"  Quality range: {summary['min_quality']:.3f} - {summary['max_quality']:.3f}"
    )
    ...
    for ticker, details in report["ticker_details"].items():
        ...
        if trend:
            trend_symbol = (
                ...
                else "📉" if trend["trend_direction"] == "degrading" else "➡️"
            )
            print(
                f"    Trend: {trend_symbol} {trend['trend_direction']} (slope: {trend['trend_slope']:.4f})"
            )
