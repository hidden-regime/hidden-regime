...
def create_realistic_sample_data(ticker_name="SAMPLE", n_days=500):
    ...
    for i in range(1, n_days):
        ...
        if regime == 0:  # Bear market
            ...
            daily_return = np.random.normal(0.0012, 0.018)   # Positive drift, moderate vol
...
def main():
    ...
    with open(report_filename, 'w') as f:
        ...
        f.write(f"Analyzed {len(results)} stocks using Hidden Markov Model regime detection.\\n\\n")
        
        for ticker, result in results.items():
            f.write(f"### {ticker} Analysis\\n\\n")
            f.write(f"- **Current Regime**: {['Bear', 'Sideways', 'Bull'][result['current_regime']]}\\n")
            ...
            for regime, stats in result['regime_stats'].items():
                f.write(f"- **{stats['name']}**: {stats['days']} days ({stats['percentage']:.1f}%)\\n")
        ...
        f.write("## Methodology\\n\\n")
        f.write("This analysis uses a 3-state Hidden Markov Model to detect market regimes:\\n\\n")
        ...
        f.write("The model analyzes log returns to identify regime patterns and transitions.\\n\\n")
    ...
    for ticker, result in results.items():
        try:
            ...
            ax1.plot(raw_data.index, raw_data['close'], linewidth=1.5, color='black', alpha=0.7)
                ...
                if mask.sum() > 0:
                    ...
                    for date in regime_dates:
                        ax1.axvspan(date, date + pd.Timedelta(days=1), 
                                   color=regime_colors[i], alpha=0.2)
            
            ax1.set_title(f'{ticker} Price Chart with Detected Regimes', fontsize=14, fontweight='bold')
            ...
            for i, regime in enumerate([0, 1, 2]):
                ...
                if mask.sum() > 0:
                    ax2.scatter(model_output.index[mask], [regime] * mask.sum(), 
                               c=regime_colors_solid[i], alpha=0.8, s=15,
                               label=f"{regime_names[i]} ({mask.sum()} days)")
            
            ax2.set_title('Market Regime Detection Timeline', fontsize=14, fontweight='bold')
    ...
    for ticker, result in results.items():
        ...
        if result['regime_returns']:
            best_regime = max(result['regime_returns'].keys(), 
            ...
            print(f"  Best Performing Regime: {best_regime_name} ({best_return:.3%} daily)")
