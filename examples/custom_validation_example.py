...
def demonstrate_asset_type_validation():
    ...
    crypto_dates = pd.date_range('2024-01-01', '2024-01-31', freq='D')  # Daily including weekends
    ...
    missing_days = np.random.choice(len(penny_dates), size=len(penny_dates)//4, replace=False)
    penny_trading_days = [date for i, date in enumerate(penny_dates) if i not in missing_days]
    
    penny_returns = np.random.normal(0.001, 0.12, len(penny_trading_days))  # Very high volatility
    ...
    datasets['Penny Stock'] = pd.DataFrame({
        ...
        'volume': np.random.lognormal(10, 1.5, len(penny_trading_days))  # Lower volume
    })
    
    # Blue chip data (stable, high liquidity)
    bluechip_dates = pd.bdate_range('2024-01-01', '2024-01-31')
    bluechip_returns = np.random.normal(0.0008, 0.015, len(bluechip_dates))  # Low volatility
    ...
    datasets['Blue Chip'] = pd.DataFrame({
        ...
        'volume': np.random.lognormal(16, 0.3, len(bluechip_dates))  # High, stable volume
    })
...
def demonstrate_trading_strategy_validation():
    ...
    base_returns[outlier_indices] = np.random.choice([-0.15, 0.18], size=5)  # ±15-18% returns
    ...
    for strategy_name, config in strategy_configs.items():
        validator = hr.DataValidator(config)
        validation_result = validator.validate_data(test_dataset, f"STRATEGY_{strategy_name.upper()}")
    ...
    if score_difference > 0.1:
        print(f"  💡 Insight: Conservative strategy rejects this data, aggressive accepts")
...
def demonstrate_custom_validation_rules():
    ...
    problematic_data = pd.DataFrame({
        ...
        'price': [100, 102, np.nan, np.nan, 98, 200, 95, 97, np.nan, 300, 
                 50, 105, 108, np.nan, 400, 90, 85, np.nan, np.nan, 110, 
                 115, 120],  # Mix of missing values and extreme changes
        'log_return': [0.02, np.nan, np.nan, -0.02, 0.5, -0.6, 0.02, np.nan, 2.0, -0.8,
                      0.8, 0.03, np.nan, 3.0, -0.9, -0.05, np.nan, np.nan, 0.27, 
                      0.04, 0.041],  # Extreme returns and missing values
    ...
    for config_name, config in configs.items():
        validator = hr.DataValidator(config)
        validation_result = validator.validate_data(problematic_data, f"CUSTOM_{config_name.replace(' ', '_').upper()}")
        ...
        if config_name == 'Algorithmic Trading':
            if not validation_result.is_valid:
                print(f"  🤖 Algo Trading Recommendation: REJECT - Data unsuitable for algorithmic trading")
            else:
                print(f"  🤖 Algo Trading Recommendation: ACCEPT - Data meets strict algorithmic requirements")
        else:  # Research
            if validation_result.quality_score < 0.2:
                print(f"  🔬 Research Recommendation: Data has significant issues but may be usable for research")
            else:
                print(f"  🔬 Research Recommendation: Data acceptable for backtesting and research")
