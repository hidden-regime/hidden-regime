...
def create_correlated_sample_data(assets, n_days=300, correlation=0.7):
    ...
    for asset in assets:
        ...
        for i in range(1, n_days):
            ...
            if regime == 0:  # Bear
                daily_return = np.random.normal(-0.001 * growth_bias, 0.025 * vol_multiplier)
            elif regime == 1:  # Sideways
                daily_return = np.random.normal(0.0001 * growth_bias, 0.015 * vol_multiplier)
            else:  # Bull
                daily_return = np.random.normal(0.0015 * growth_bias, 0.020 * vol_multiplier)
...
def main():
    ...
    with open(report_filename, 'w') as f:
        ...
        f.write(f"Comparative regime analysis across {len(regime_results)} market segments using ")
        f.write("Hidden Markov Model detection. Analysis identifies synchronization patterns ")
        ...
        f.write("Correlation between regime sequences (1.0 = perfect correlation):\\n\\n")
        ...
        if len(high_sync_periods) > 0:
            ...
            f.write(f"- {len(high_sync_periods)} days out of {len(sync_scores)} total\\n")
            f.write(f"- {len(high_sync_periods)/len(sync_scores):.1%} of analysis period\\n\\n")
        
        f.write("## Regime Distribution by Asset\\n\\n")
        for asset_code, result in regime_results.items():
            f.write(f"### {result['name']}\\n\\n")
            regime_counts = result['model_output']['predicted_state'].value_counts().sort_index()
        ...
        if avg_sync > 0.7:
            f.write("Markets show **high synchronization**, suggesting strong macro factors ")
            f.write("driving regime changes across all segments.\\n\\n")
        elif avg_sync > 0.5:
            f.write("Markets show **moderate synchronization**, with some common drivers ")
            f.write("but also segment-specific patterns.\\n\\n")
        else:
            f.write("Markets show **low synchronization**, indicating strong sector-specific ")
        ...
        if max_corr < 0.5:
            f.write("Low regime correlations suggest **good diversification potential** ")
            f.write("across these market segments.\\n\\n")
        elif min_corr > 0.8:
            f.write("High regime correlations suggest **limited diversification benefits** ")
            f.write("during regime transitions.\\n\\n")
        else:
            f.write("Mixed regime correlations suggest **selective diversification opportunities** ")
    ...
    try:
        ...
        fig, axes = plt.subplots(len(regime_results) + 1, 1, 
                                figsize=(16, 4 * (len(regime_results) + 1)))
        ...
        for i, (asset_code, result) in enumerate(regime_results.items()):
            ...
            ax2 = ax.twinx()
            ax2.plot(result['data'].index, result['data']['close'], 
            ...
            for regime in [0, 1, 2]:
                ...
                if mask.sum() > 0:
                    ax.scatter(model_output.index[mask], [regime] * mask.sum(),
        ...
        ax_sync.axhline(y=0.5, color='orange', linestyle='--', alpha=0.7, label='50% Sync')
