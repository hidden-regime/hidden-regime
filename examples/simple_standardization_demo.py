...
def test_3state_standardization():
    ...
    if hasattr(hmm, '_state_standardizer') and hmm._state_standardizer:
        ...
        for i in range(hmm.n_states):
            ...
            print(f"  {regime_name}: μ={mean:.4f} ({annualized_return:.1%}/year), σ={std:.4f} ({annualized_vol:.1%}/year)")
...
def test_economic_validation(hmm):
    ...
    print(f"Mean ordering correct: {'✅' if details['mean_ordering_correct'] else '❌'}")
    print(f"Volatility reasonable: {'✅' if details['volatility_reasonable'] else '❌'}")
...
def test_regime_inference(hmm, returns):
    ...
    for i in range(min(10, len(returns))):
        ...
        print(f"  Day {i+1}: return={return_val:7.4f} → State {state} (conf: {confidence:.3f})")
    ...
    for idx in neg_idx:
        ...
        print(f"    Day {idx+1}: {return_val:.4f} → State {state} (conf: {confidence:.3f})")
