...
# Import using factory pattern (recommended)
...
def main():
    ...
    try:
        # Load market data using factory (recommended)
        print(f"\n Loading market data for {ticker}...")
        data_loader = component_factory.create_data_component(
    ...
    # Create observation component using factory (recommended)
    print("\n🔍 Creating observations...")
    observation_config = FinancialObservationConfig(generators=["log_return"])
    observation_component = component_factory.create_observation_component(observation_config)

    # Generate observations
    observations = observation_component.update(raw_data)
    print(f" Generated {len(observations)} observations")

    # Create and train HMM model using factory (recommended)
    print("\n🤖 Training HMM model...")
    model_config = HMMConfig(n_states=3, random_seed=42)
    hmm_model = component_factory.create_model_component(model_config)
