...
def validate_portfolio_data(data_dict):
    ...
    quality_scores = {ticker: result.quality_score 
                     for ticker, result in validation_results.items()}
    
    print(f"\nData Quality Summary:")
    print(f"  Average quality score: {np.mean(list(quality_scores.values())):.3f}")
    print(f"  Best quality: {max(quality_scores, key=quality_scores.get)} ({max(quality_scores.values()):.3f})")
    print(f"  Worst quality: {min(quality_scores, key=quality_scores.get)} ({min(quality_scores.values()):.3f})")
    ...
    for ticker in sorted(quality_scores.keys()):
        ...
        print(f"  {status} {ticker:6s}: Quality {quality:.3f} | Issues: {issues} | Warnings: {warnings_count}")
...
def process_portfolio_data(data_dict):
    ...
    try:
        ...
        for ticker, processed_data in processed_dict.items():
            ...
            print(f"  - {ticker}: {len(original_data)} → {len(processed_data)} rows, "
...
def analyze_portfolio_volatility(processed_dict):
    ...
    print(f"\nVolatility Analysis:")
    print(f"{'Ticker':<8} {'Daily Vol':<10} {'Annual Vol':<11} {'Max Return':<11} {'Min Return':<11} {'Skew':<7} {'Kurt':<7}")
    print("-" * 70)
    
    for ticker in sorted(volatility_stats.keys()):
        stats = volatility_stats[ticker]
        print(f"{ticker:<8} {stats['daily_vol']:<10.4f} {stats['annualized_vol']:<11.1%} "
              f"{stats['max_daily_return']:<11.3f} {stats['min_daily_return']:<11.3f} "
              f"{stats['return_skewness']:<7.2f} {stats['return_kurtosis']:<7.2f}")
    
    # Portfolio-level analysis
    all_returns = pd.concat([data['log_return'].dropna() for data in processed_dict.values()], ignore_index=True)
    ...
    if len(all_returns) > 0:
        ...
        print(f"  Average daily volatility: {portfolio_vol:.4f} ({portfolio_annual_vol:.1%} annualized)")
        ...
        daily_vols = {ticker: stats['daily_vol'] for ticker, stats in volatility_stats.items()}
...
def generate_portfolio_insights(data_dict, processed_dict, validation_results):
    ...
    if avg_quality > 0.8:
        insights.append(f"✓ Excellent data quality across portfolio (avg: {avg_quality:.3f})")
    elif avg_quality > 0.6:
        insights.append(f"⚠ Moderate data quality across portfolio (avg: {avg_quality:.3f})")
        recommendations.append("Consider data cleaning or filtering for low-quality stocks")
    else:
        insights.append(f"✗ Poor data quality across portfolio (avg: {avg_quality:.3f})")
    ...
    if data_lengths:
        ...
        if min_length < 50:
            insights.append(f"⚠ Some stocks have limited data (minimum: {min_length} days)")
            recommendations.append("Extend date range for more robust analysis")
        elif min_length > 200:
            insights.append(f"✓ Comprehensive data coverage (minimum: {min_length} days)")
    ...
    if len(processed_dict) > 1:
        ...
        if returns_data:
            ...
            if portfolio_vol > 0.4:  # >40% annualized volatility
                insights.append(f"⚠ High portfolio volatility ({portfolio_vol:.1%} annualized)")
                recommendations.append("Consider risk management strategies for high-volatility portfolio")
            elif portfolio_vol < 0.15:  # <15% annualized volatility
                insights.append(f"✓ Low portfolio volatility ({portfolio_vol:.1%} annualized)")
            else:
                insights.append(f"✓ Moderate portfolio volatility ({portfolio_vol:.1%} annualized)")
...
def main():
    ...
    try:
        ...
        print("Use the processed data for regime detection and trading strategy development.")
