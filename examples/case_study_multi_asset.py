    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            if 'final_comparison' in ticker_results:
                ...
                if 'comparison_summary' in comparison and 'strategy_ranking' in comparison['comparison_summary']:
                    best_strategy = comparison['comparison_summary']['strategy_ranking'][0]
...
def generate_multi_asset_summary(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if all_metrics['sharpe_ratios']:
        summary['average_metrics'] = {
            'avg_sharpe_ratio': sum(all_metrics['sharpe_ratios']) / len(all_metrics['sharpe_ratios']),
            'avg_total_return': sum(all_metrics['total_returns']) / len(all_metrics['total_returns']),
            'avg_max_drawdown': sum(all_metrics['max_drawdowns']) / len(all_metrics['max_drawdowns']),
            'avg_win_rate': sum(all_metrics['win_rates']) / len(all_metrics['win_rates'])
        }

        # Find best and worst performers
        if asset_performances:
            # Best Sharpe ratio
            best_sharpe_ticker = max(asset_performances.keys(),
            ...
            # Best total return
            best_return_ticker = max(asset_performances.keys(),
...
def print_multi_asset_summary(multi_asset_results: Dict[str, Any]):
    ...
    if 'best_performers' in summary:
        ...
        if 'sharpe_ratio' in best:
            print(f"  Highest Sharpe: {best['sharpe_ratio']['ticker']} ({best['sharpe_ratio']['value']:.3f})")

        if 'total_return' in best:
            print(f"  Highest Return: {best['total_return']['ticker']} ({best['total_return']['value']:.2%})")

        if 'max_drawdown' in best:
            print(f"  Lowest Drawdown: {best['max_drawdown']['ticker']} ({best['max_drawdown']['value']:.2%})")
