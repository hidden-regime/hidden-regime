...
def main():
    ...
    try:
        ...
        print(
            f"  Price range: ${data['price'].min():.2f} to ${data['price'].max():.2f}"
        )
    ...
    try:
        ...
        print(
            f"✓ Processed data - added {len(processed_data.columns) - len(custom_data.columns)} new columns"
        )

        # Get data summary
        summary = preprocessor.get_data_summary(processed_data)
        print(
            f"✓ Data summary: {summary['n_observations']} observations over {summary['date_range']['days']} days"
        )

        # Validate processed data
        custom_validation = validator.validate_data(processed_data, "MSFT")
        print(
            f"✓ Custom validation - Quality score: {custom_validation.quality_score:.2f}"
        )
    ...
    try:
        ...
        for ticker, stock_data in processed_multi.items():
            ...
            print(
                f"  - {ticker}: Quality {validation.quality_score:.2f}, Valid: {validation.is_valid}"
            )
